...
Lifts every SDK import into *lazy* accessors so the rest of det
(``import det``, ``det list``, the DuckDB destination, every test that does
...
def _bigquery_module() -> Any:
    ...
    det`` without ``[bigquery]`` installed does not crash. A missing SDK
...
def _service_account_credentials(path: str) -> Any:
    ...
    Credentials are *referenced* by path on disk — det never logs the path's
...
class BigQueryClient:
    ...
    The two clients are built by :func:`build_client` from a :class:`~det.types.Config`.
    """
...
class BQConn:
    ...
    * :attr:`runs_table_ready` — same flag for ``_det_runs`` (docs/09 §4).
    """
...
# --------------------------------------------------------------------------
# Client construction — from a det Config to a live BigQueryClient
# --------------------------------------------------------------------------
