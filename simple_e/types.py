"""simpl.E contract types — the core type definitions every module imports.

This module (``simple_e/types.py``) is the authoritative, in-code expression
of the simpl.E connector contract. It contains *only* type definitions and their small intrinsic
...
class _StrEnum(StrEnum):
    ...
    def parse(cls, value: Any) -> Any:
        ...
        which is the message ``simple-e validate`` surfaces to the author.
        """
...
class Capability(Enum):
    ...
    STATE = "state"
    """Destination can host the ``_simple_e_state`` table itself (Tier A)."""
    MERGE = "merge"
...
class CursorType(_StrEnum):
    ...
    examples and the ``_simple_e_state.cursor_type`` column.
    """
...
class Schema:
    ...
    SYNCED_AT_COLUMN: ClassVar[str] = "_simple_e_synced_at"
    ...
    def synced_at_field() -> Field:
        """The engine-appended ``_simple_e_synced_at`` column — docs/03 §2.2.1.
    ...
    def with_synced_at(self) -> Schema:
        """Return a new :class:`Schema` with ``_simple_e_synced_at`` appended.
...
class State:
    ...
    ``state_blob`` JSON column of ``_simple_e_state`` after batches durably
...
class StateBackend(Protocol):
    ...
    Object-storage destinations cannot host the ``_simple_e_state`` table, so
...
@dataclass
class StateRecord:
    """One ``_simple_e_state`` row — the per-stream resume point.
    ...
    # excluded — it is recoverable by joining ``_simple_e_runs`` on
    ...
    def to_row(self) -> dict[str, Any]:
        """Serialize to a plain dict matching the ``_simple_e_state`` columns.
    ...
    def from_row(cls, row: Mapping[str, Any]) -> StateRecord:
        """Deserialize a ``_simple_e_state`` row read back from a destination.
...
class RunResult:
    ...
    run record (``_simple_e_runs`` row / ``.simple_e/runs/<run_id>.json``).
