...
production warehouse detx was always meant to land in — built to the same
contract as the DuckDB reference (see ``detx/destinations/duckdb/``), with
...
# ``detx/engine/runner.py::_stream_transaction``). A future opt-in
...
from detx.destinations.bigquery.client import (
    BigQueryClient,
    BQConn,
    build_client,
    run_with_retries,
...
from detx.types import Field, FieldType


# # NOTE: ``PartitionDriftError`` is intentionally a destination-local
# exception class, not the engine's :class:`~detx.engine.runner.EngineError`.
# The task spec asks for an EngineError on partition drift, but a
# destination importing from ``detx.engine.runner`` would introduce a reverse
...
# folds it into a ``FAILED`` :class:`~detx.types.RunResult`, so a
...
class PartitionDriftError(RuntimeError):
    ...
    :class:`~detx.types.PartitionConfig` requests — see :func:`compare_partition`
...
# ``detx.destinations.bigquery.client`` module. Going through the attribute
...
def capabilities() -> set[Capability]:
    ...
    * ``STATE`` — BigQuery hosts the ``_detx_state`` table itself (Tier A,
    ...
    * ``RUN_RECORDS`` — BigQuery hosts the ``_detx_runs`` audit table
      alongside ``_detx_state``, queryable with plain SQL (docs/09 §4).
...
def max_concurrent_writes(config: Config) -> int:
    ...
    ``detx run --tag X --threads 4`` against five BigQuery configs caps at
...
def open(config: Config) -> BQConn:  # noqa: A001 — hook name is fixed by the contract
    ...
    * ``job_timeout_seconds`` / ``retry_max_attempts`` / ``retry_backoff_seconds``
    ...
    — detx never creates buckets, that is an operator concern (region,
    ...
    staging_prefix = str(config.get("staging_prefix") or "detx/staging")
...
def _verify_staging_bucket(client: BigQueryClient) -> None:
    ...
    detx does not create the staging bucket: region / retention / lifecycle
...
def ensure_schema(conn: BQConn, stream: StreamMeta) -> None:
    ...
    The engine appends ``_detx_synced_at`` to every record (docs/03 §2.2.1);
...
def write_batch(conn: BQConn, batch: Batch, stream: StreamMeta) -> int:
    ...
    * ``replace`` — first batch of the run uses ``WRITE_TRUNCATE`` (clears
    ...
    The engine sets ``_detx_synced_at`` on every record; this hook fills
...
def _stamp_synced_at(batch: Batch) -> list[dict[str, Any]]:
    """Return a copy of ``batch`` with ``_detx_synced_at`` set on every record.
...
def _arrow_type_for(field_type: FieldType, pa: Any) -> Any:
    """Map a detx :class:`FieldType` to a pyarrow type — the Parquet column type."""
    ...
    if field_type is FieldType.TIMESTAMP:
        ...
        # engine stamps ``_detx_synced_at`` in UTC; aware datetimes carry the
...
def _merge_via_staging(
    ...
    :func:`~detx.destinations.bigquery.ddl.merge_sql` and contains only
...
# --------------------------------------------------------------------------
# read_state / commit_state — the _detx_state table — docs/05 §5
# --------------------------------------------------------------------------


def _state_schema() -> Iterator[tuple[str, FieldType]]:
    """The canonical ``_detx_state`` column set — mirrors ``StateRecord.to_row()``.
...
def _ensure_state_table(conn: BQConn) -> None:
    """Create ``_detx_state`` lazily on first use — docs/05 §5.1.
...
def read_state(conn: BQConn, connector: str) -> list[StateRecord]:
    ...
    :class:`StateRecord` per ``_detx_state`` row whose ``connector`` matches
...
def commit_state(conn: BQConn, run_id: str, records: list[StateRecord]) -> None:
    """Upsert the run's :class:`StateRecord` set into ``_detx_state`` — docs/05 §5.
...
# --------------------------------------------------------------------------
# write_run_record — the _detx_runs audit table — docs/09 §4
# --------------------------------------------------------------------------


def _runs_schema() -> Iterator[tuple[str, FieldType, str]]:
    """The canonical ``_detx_runs`` column set — mirrors the DuckDB destination.
...
def _ensure_runs_table(conn: BQConn) -> None:
    """Create ``_detx_runs`` lazily on first use — docs/09 §4."""
...
def write_run_record(conn: BQConn, record: RunRecord) -> None:
    """Persist one :class:`RunRecord` to ``_detx_runs`` — docs/09 §4.
