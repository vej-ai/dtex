...
# engine (detx/engine/config.py::build_config) constructs a single
# connector-level :class:`~detx.types.Config` and injects only that into a
...
the engine injects a :class:`~detx.types.Cursor` (docs/03 §3.2). Both
...
from detx.sources.postgres.client import (
    close_cursor_sql,
    connect,
    declare_cursor_sql,
    fetch_forward_sql,
...
def _extract_full_scan(
    *,
    ...
    """
    batch_size = int(config.batch_size)
    cursor_name = f"_detx_pg_{uuid.uuid4().hex[:12]}"
