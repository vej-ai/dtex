...
"""The ``dtex`` command-line interface — a thin shell over the engine.

docs/07 §The CLI is a thin shell over a real Python library: every command here
parses arguments, calls the engine's *public* surface — :func:`dtex.run`,
the discovery functions in :mod:`dtex.engine.discovery`, and the config loader
in :mod:`dtex.engine.configs` — and formats the result. There is **no** run
...
longer runnable because it doesn't say where to write. ``dtex list`` /
``dtex validate`` cover sources, destinations, AND configs; ``dtex init``
scaffolds the new layout (``sources/`` + ``destinations/`` + ``configs/``);
``dtex new`` has three subcommands (``source``, ``destination``, ``config``);
``dtex state`` is keyed by config (which resolves to a source for the actual
state lookup).

Command surface:

* ``dtex run -p <config> [--target T] [--select S] [--full-refresh]
  [--param k=v] [--destination-param k=v]`` — extract + load one config.
* ``dtex list [--kind {{source,destination,config}}]`` — list discoverable
  components.
* ``dtex validate`` — discovery-time validation of every source, destination,
  and config.
...
from dtex.cli._format import (
    print_multi_run_summary,
    print_run_result,
    render_table,
)
from dtex.cli._runs import get_run as runs_get
...
@click.version_option(version=dtex.__version__, prog_name="dtex")
def cli() -> None:
    """dtex — a simple, open-source extract-load tool.
    ...
    if tag is not None:
        ...
        if params:
            _fail(
                ...
                "apply to every matched config silently); use `dtex run -p "
...
@click.option(
    ...
    "tags: drives `dtex run --tag`; source/destination tags are catalog "
    "metadata only).",
)
@click.option(
    "--project-dir",
...
def list_components(
    ...
    ``dtex run --tag`` selects on). A section with no matches still shows
...
def init(directory: Path, force: bool, with_: tuple[str, ...]) -> None:
    ...
    try:
        ...
        _fail(str(exc), code=2)
        return  # unreachable.
    click.echo(click.style(f"created dtex project at {root}", fg="green"))
    click.echo("next:")
    click.echo("  dtex new source my_source")
    click.echo("  dtex new config my_pipeline")
    click.echo("  dtex validate")
    click.echo("  dtex run -p my_pipeline")
...
def new_source(name: str, project_dir: Path | None) -> None:
    ...
    click.echo("  dtex new config <name>   # bind this source to a destination")
    click.echo("  dtex validate")
...
@cli.group()
def state() -> None:
    """Inspect and reset incremental state (the destination's _dtex_state)."""
...
def state_reset(
    ...
    Deletes the rows in ``_dtex_state`` for the config's source (or one
...
@cli.group()
def runs() -> None:
    """Inspect run history (the destination's _dtex_runs + per-run JSONL log)."""
...
def runs_list_cmd(
    ...
    """List recent runs from the destination's ``_dtex_runs`` table.
...
def runs_show_cmd(
    ...
    Looks up ``<run_id>`` in ``_dtex_runs`` (the queryable summary) and
    reads ``.dtex/logs/<run_id>/run.jsonl`` (the narrative). Accepts the
    ...
    if record is not None:
        ...
        click.echo(click.style(f"no _dtex_runs row for {run_id!r}", fg="yellow"))
...
def main() -> None:
    """Entry point for the ``dtex`` console script (pyproject scripts).
