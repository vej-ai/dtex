...
# ``{config, state, cursor, log}`` (``detx.registry.STREAM_INJECTABLES``).
...
from detx.sources.shiphero.client import ShipHeroClient
from detx.sources.shiphero.pagination import paginate
from detx.sources.shiphero.queries import (
    ...
    for win_from, win_to in date_windows(start, step_days=int(config.step_days)):
        ...
        for raw_record in paginate(
            ...
            if value is not None:
                ...
                # destination's ``_detx_state.cursor_value`` is a ``JSON``
