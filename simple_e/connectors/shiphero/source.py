...
# ``{config, state, cursor, log}`` (``simple_e.registry.STREAM_INJECTABLES``).
...
from simple_e.connectors.shiphero.client import ShipHeroClient
from simple_e.connectors.shiphero.pagination import paginate
from simple_e.connectors.shiphero.queries import (
    ...
    for win_from, win_to in date_windows(start, step_days=int(config.step_days)):
        ...
        for raw_record in paginate(
            ...
            if value is not None:
                ...
                # destination's ``_simple_e_state.cursor_value`` is a ``JSON``
