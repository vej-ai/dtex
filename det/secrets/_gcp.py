...
``profiles.yml`` without any other wiring::
    ...
    pip install 'det[gcp-secrets]'

The resolver is built on first use by stage 9a's lazy instantiation
(:func:`det.secrets.resolvers._get_resolver`); the SDK client itself is
...
(see :mod:`det.destinations.bigquery.client`).
...
# 429 with exponential backoff; layering det-side retries would double
...
logger = logging.getLogger("det.secrets")
...
def _lazy_import_gcp_secrets() -> Any:
    ...
    :func:`det.sources.filesystem.backends._lazy_import_gcs` and
    :func:`det.destinations.bigquery.client._bigquery_module` — the SDK is
    NOT imported at ``det`` package import time, so a base install (or one
...
class GcpSecretManagerResolver:
    ...
    * The cache is per-instance, and the instance is itself cached
      ...
      SDK client per ``det`` process, however many ``secret://gcp-secret-manager/...``
      references the run consumes.

    Errors:

    * Malformed path (does not match :data:`_RESOURCE_NAME`) →
      :class:`~det.secrets.SecretResolutionError` with the validation
      message.
    * ``google.api_core.exceptions.PermissionDenied`` →
      :class:`~det.secrets.SecretResolutionError` with "permission denied
    ...
    * ``google.api_core.exceptions.NotFound`` →
      :class:`~det.secrets.SecretResolutionError` with "secret X version
      Y not found".
    * Any other SDK error → :class:`~det.secrets.SecretResolutionError`
    ...
    def resolve(self, path: str, field: str | None) -> str:
        ...
        Returns:
            ...
            :class:`~det.secrets.SecretResolutionError` (the underlying
        ...
        try:
            ...
            raise SecretResolutionError(
                ...
                f"det requires resolved secret values to be strings"
            ) from exc
