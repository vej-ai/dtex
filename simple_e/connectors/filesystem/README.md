...
A baked simpl.E source that reads flat files (CSV, JSONL, Parquet) from a
...
record as `_simple_e_file_cursor` (a synthetic field the source attaches), so
the engine's standard `Cursor` + `_simple_e_state` machinery just works.
...
| `gs://`   | `GcsBackend`     | `simple-e[gcs]`      |
| `s3://`   | `S3Backend`      | `simple-e[s3]`       |

Parquet files need pyarrow regardless of backend: `simple-e[parquet]`.
...
`_simple_e_file_cursor` (STRING)** in it (the contract requires any declared
schema to contain the cursor field, see `simple_e/types.py::StreamDef`).
