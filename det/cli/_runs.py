"""Run-history inspection for ``det runs`` — docs/09 §4, stage 8a.

``det runs list`` queries the destination's ``_det_runs`` audit table; ``det
...
same way :mod:`det.cli._state` does for ``det state list``.
...
# ``_det_runs`` alongside ``_det_state``). Different configs in one project
...
#
#   ``det runs list -p <config>`` (config name disambiguates the destination)
#
# This mirrors ``det state list`` exactly and avoids inventing a multi-
...
# NOTE: like :mod:`det.cli._state`, this module reaches past the destination
...
class RunRow:
    """One row from ``_det_runs`` — the shape the CLI prints + admin reads."""
...
def _resolve(
    ...
    *,
    ...
    The same stage-1-through-3 path as :func:`det.run`, stopping once the
...
def _row_to_runrow(row: tuple[Any, ...]) -> RunRow:
    """Build a :class:`RunRow` from a ``_det_runs`` row in column order.
...
def _query_runs(conn: Any, *, run_id: str | None, limit: int | None) -> list[RunRow]:
    """SELECT ``_det_runs`` rows, optionally filtered to a single ``run_id``."""
    ...
    try:
        if Capability.RUN_RECORDS not in dest.capabilities:
            raise StateError(
                ...
                f"Capability.RUN_RECORDS; no _det_runs table is written"
            )
...
def get_run(
    ...
    *,
    ...
    log_path = dest.project_root / ".det" / "logs" / run_id / "run.jsonl"
