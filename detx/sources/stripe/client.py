...
Plain module — no decorators, no detx contract — exactly the "API client"
...
:mod:`detx.sources.stripe.pagination`, kept separate so this file is
...
class StripeClient:
    ...
    builds a client from :class:`~detx.types.Config` at the top of the
    stream, then drives the cursor pagination loop in
    :mod:`detx.sources.stripe.pagination` against it. A long-lived
