"""Postgres type → :class:`~det.types.FieldType` mapping + schema introspection.

docs/03 §2.2.1 fixes the det :class:`~det.types.FieldType` set; this
...
# runtime hook. det's contract gives a ``@stream`` function no way to hand
...
from det import Field, FieldMode, FieldType, Schema

if TYPE_CHECKING:
    import psycopg

# The complete, fixed mapping from a normalised Postgres type name to a
# det :class:`~det.types.FieldType`. Keys are the lower-cased
...
_PG_TO_FIELD_TYPE: dict[str, FieldType] = {
    ...
    # uuid → STRING per the task spec (no native UUID field type in det)
    "uuid": FieldType.STRING,
}


def postgres_to_field_type(pg_type: str) -> FieldType:
    """Map one Postgres type name to a det :class:`~det.types.FieldType`.
...
def introspect_schema(
    ...
    :class:`~det.types.FieldMode` (``NO`` → ``REQUIRED``, anything else →
    ``NULLABLE``).
