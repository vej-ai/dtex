...
:data:`~simple_e.registry.STREAM_INJECTABLES`); the engine injects a logger
...
def build_logger(run_id: str, secret_values: Iterable[str] = ()) -> logging.Logger:
    ...
    """
    logger = logging.getLogger(f"simple_e.run.{run_id}")
    logger.setLevel(logging.INFO)
    # A run logger emits on its own handler, not the root logger's — keep it
    # self-contained so importing simpl.E never reconfigures a host app's logging.
    ...
    if not logger.handlers:
        ...
        handler.setFormatter(
            logging.Formatter("%(asctime)s [%(levelname)s] simple_e: %(message)s")
        )
