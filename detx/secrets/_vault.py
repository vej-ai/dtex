...
* ``<mount-path>/<kv-path>`` — the full Vault read path, opaque to detx.
...
* ``<field>`` — REQUIRED. Vault's KV engines return a JSON object;
  ...
  raises a clear :class:`~detx.secrets.SecretResolutionError`.

Auto-registered via ``[project.entry-points."detx.secret_resolvers"]``::

    pip install 'detx[vault]'

The resolver is built on first use by stage 9a's lazy instantiation
(:func:`detx.secrets.resolvers._get_resolver`); the :class:`hvac.Client`
...
2. Grant the token detx runs as a policy with
...
4. Export ``VAULT_ADDR`` (the Vault URL) and ``VAULT_TOKEN`` (the auth
   token) in the environment detx runs in. These are the same env vars
...
def _lazy_import_hvac() -> Any:
    ...
    imported at ``detx`` package import time, so a base install keeps
...
class VaultResolver:
    ...
    * Effective lifetime: one ``hvac.Client`` per ``detx`` process,
      however many ``secret://vault/...`` references the run consumes.

    Errors:

    * Missing ``#field`` → :class:`~detx.secrets.SecretResolutionError`
      with the contract message — fired BEFORE SDK import.
    * Empty path → :class:`~detx.secrets.SecretResolutionError` — fired
      BEFORE SDK import.
