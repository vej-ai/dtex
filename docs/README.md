# detx — Design Handbook

**detx** (short for "data extraction tool") is an open-source, self-hosted Python **extract-load (EL)** tool. The pitch in one line: **a CLI-first, dbt-shaped extract-load tool — pipelines as configs, connectors as folders, no UI blackbox.**
...
If you are evaluating detx, read 00–02 and 10. If you are writing a connector, 03–05 are the working reference. If you are running it, 06, 07, and 12. If you are reviewing what is still open, start with 11.

## Table of contents

| # | Section | What it covers |
|---|---|---|
| 00 | [Vision & Naming](./00-vision-and-naming.md) | Why detx exists, the "dbt for EL" thesis, the simplicity mandate, and the naming. |
| 01 | [Landscape & Comparison](./01-landscape-and-comparison.md) | Where detx sits vs. Airbyte, Fivetran, dlt, Meltano, and custom scripts. |
...
| 05 | [Destinations and State](./05-destinations-and-state.md) | The destination interface, the baked catalog, schema handling, write dispositions, and the `_detx_state` design with the capability-tier model. |
| 06 | [Project Anatomy](./06-project-anatomy.md) | `detx_project.yml`, the `sources/`, `destinations/`, and `configs/` directories, `.detx/`, the destination-keyed `profiles.yml`. |
| 07 | [CLI and Library API](./07-cli-and-library-api.md) | The full `detx` command surface, exit codes, the importable Python library, orchestrator use, config precedence. |
...
| 09 | [Logging and Observability](./09-logging-and-observability.md) | Structured per-run logs, run lifecycle events, the run record and `_detx_runs`, log levels and redaction. |
