...
A simpl.E source connector that extracts ShipHero fulfillment data via the
...
simple-e run shiphero --target prod
```
...
- `batch_size` (default 200) — records per simpl.E batch (the destination
...
```sh
SHIPHERO_REFRESH_TOKEN=... simple-e run shiphero --target dev \
...
- `get_checkpoint` / `save_checkpoint` → the engine's `Cursor` + `_simple_e_state`.
