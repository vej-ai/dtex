...
result objects (:class:`~detx.types.RunResult`,
:class:`~detx.types.StateRecord`) and discovery objects into aligned,
...
from detx.engine import last_run_tag_parallelism
from detx.types import RunResult, RunStatus, StreamStatus
...
def print_multi_run_summary(tag: str, results: list[RunResult]) -> None:
    """Print a one-table summary of a ``detx run --tag`` multi-run — stage 8d.
    ...
    # matching :class:`~detx.types.RunResult.to_dict`. The full traceback
