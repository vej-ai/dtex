...
* the detx :class:`~detx.types.FieldType` → DuckDB native-type mapping
  (docs/05 §3.1);
...
The mapping is total over :class:`~detx.types.FieldType`'s eight members.
"""
...
from detx.types import FieldType, Schema
...
"""detx :class:`FieldType` → DuckDB column type — docs/05 §3.1.
...
def duckdb_type(field_type: FieldType) -> str:
    """Return the DuckDB native type for a detx :class:`FieldType` — docs/05 §3.1.
...
# (``_detx_state``, ``_detx_synced_at``) start with one.
...
def validate_identifier(name: str, *, kind: str = "identifier") -> str:
    ...
    detx table and column names come from ``register.yaml`` and from source
