"""Postgres connection + safe-SQL helpers for the simpl.E Postgres connector.
...
# (simple_e/engine/discovery.py), which runs before the engine has confirmed
...
def keyset_select_sql(
    ...
    docs/03 §2.2: keyset (cursor + PK) pagination is the contract simpl.E
...
def query_select_sql(
    ...
    .. code-block:: sql
        ...
        SELECT * FROM ({user_query}) AS _simple_e_sub
        WHERE "cursor_field" > %s
