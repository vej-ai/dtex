...
def _lazy_import_pyarrow() -> Any:
    ...
    pyarrow ships with dtex's base dependencies (the BigQuery destination
    needs it). Reaching this error means the package was removed from the
    active environment — reinstall with ``pip install dtex``.
    """
    try:
        ...
        raise ImportError(
            ...
            "pyarrow ships with `pip install dtex`; if you see this, the package "
            "was removed from your environment — reinstall with `pip install dtex`."
        ) from exc
    return pq
