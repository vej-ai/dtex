...
"""``dtex secrets test`` — verify that every declared secret reference resolves.
...
# NOTE: design decision — ``dtex secrets test`` is the place where we
# DELIBERATELY resolve every reference up front, even though normal
# ``dtex run`` resolution is lazy. The whole purpose of the command is the
...
# :mod:`dtex.secrets`). That's the same cost a real run would pay.
"""
...
from dtex.cli._discovery import discover_all_configs
...
from dtex.types import PipelineConfig, SecretRef


@dataclass(frozen=True)
class ReferenceCheck:
    """One per-reference verification result for ``dtex secrets test``.
...
def _resolve_one(
    ...
    Wraps :func:`dtex.engine.config.resolve_secret_ref` so every exception
...
def check_config(
    ...
    *,
    ...
    ``default_target`` (:func:`dtex.engine.config.resolve_target_name`).
...
def check_project(
    ...
    *,
    ...
    """Drive ``dtex secrets test`` end to end — returns (checks, ok_count, fail_count).

    * ``project_dir`` — project root, or any dir under it; walked up to
      find ``dtex_project.yml``.
    ...
    * ``target_override`` — overrides the config's ``target:`` (the
    ...
    The project's ``dtex_plugins.py`` is imported BEFORE any resolution,
    matching ``dtex run``'s behavior so a project-local custom
    ...
    # registered in dtex_plugins.py would resolve as "unknown scheme".
