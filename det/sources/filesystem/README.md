...
A baked det source that reads flat files (CSV, JSONL, Parquet) from a
...
record as `_det_file_cursor` (a synthetic field the source attaches), so
the engine's standard `Cursor` + `_det_state` machinery just works.
...
`_det_file_cursor` (STRING)** in it (the contract requires any declared
schema to contain the cursor field, see `det/types.py::StreamDef`).
