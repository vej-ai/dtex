...
A dtex source connector that extracts rows from a Postgres database. Each
...
`dtex/connectors/postgres/`. A user project can shadow it by dropping a
...
`schema:`) maps to one of dtex's `FieldType` members (docs/03 §2.2.1):
...
`application_name` (default `dtex`), `connect_timeout_seconds` (default
