...
* ``<secret-id>`` — the secret name (or full ARN). Opaque to dtex.
...
— no dtex-side credential argument is passed.
...
2. Grant the IAM principal dtex runs as the
...
# path. Surfacing a base64-decoded binary as a dtex secret value would
# also conflict with downstream consumers that expect ``str``. A binary
# secret raises :class:`~dtex.secrets.SecretResolutionError` with a clear
...
# exponential backoff; layering dtex-side retries would double the
...
_INSTALL_HINT = (
    ...
    "install with `pip install dtex[aws-secrets]`"
)
...
def _lazy_import_boto3() -> Any:
    ...
    :func:`dtex.secrets._gcp._lazy_import_gcp_secrets` — the SDK is NOT
    imported at ``dtex`` package import time, so a base install (or one
...
class AwsSecretsManagerResolver:
    ...
    * Malformed path (no slash, empty region, invalid region characters,
      empty secret-id) → :class:`~dtex.secrets.SecretResolutionError`
