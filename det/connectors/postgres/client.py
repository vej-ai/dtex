"""Postgres connection + safe-SQL helpers for the det Postgres connector.
...
# (det/engine/discovery.py), which runs before the engine has confirmed
...
def keyset_select_sql(
    ...
    docs/03 §2.2: keyset (cursor + PK) pagination is the contract det
...
def query_select_sql(
    ...
    .. code-block:: sql
        ...
        SELECT * FROM ({user_query}) AS _det_sub
        WHERE "cursor_field" > %s
