...
simpl.E binds `streams[].name` in `register.yaml` to a `@stream(name=...)`
...
The YAML carries only what the simpl.E contract supports (name / table /
...
The simpl.E contract type `ParamSpec` (docs/03 §2.4) only allows scalar
