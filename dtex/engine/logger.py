...
* **JSON-lines run log** — :class:`RunLog`, a per-run file at
  ``.dtex/logs/<run_id>/run.jsonl``. The engine emits structured lifecycle
...
class RunLog:
    """One ``.dtex/logs/<run_id>/run.jsonl`` writer, with redaction baked in.

    docs/09 §3.2: every run writes a complete JSON-lines trace, independent
    of stdout verbosity, to ``.dtex/logs/<run_id>/run.jsonl`` under the
    ...
    def __init__(self, run_id: str, log_dir: Path, redactor: Redactor) -> None:
        ...
        # mode + utf-8 is the cross-platform-safe encoding the rest of dtex
...
def build_logger(
    ...
    *,
    ...
    # see :func:`~dtex.engine.run`), so ``logging.getLogger(f"dtex.run.{id}")``
    ...
    logger = logging.getLogger(f"dtex.run.{run_id}")
    logger.setLevel(logging.INFO)
    # A run logger emits on its own handler, not the root logger's — keep it
    # self-contained so importing dtex never reconfigures a host app's logging.
    ...
    if not logger.handlers:
        ...
        handler.setFormatter(
            logging.Formatter("%(asctime)s [%(levelname)s] dtex: %(message)s")
        )
