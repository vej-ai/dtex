...
"""Postgres type → :class:`~dtex.types.FieldType` mapping + schema introspection.
...
_PG_TO_FIELD_TYPE: dict[str, FieldType] = {
    ...
    # uuid → STRING per the task spec (no native UUID field type in dtex)
    "uuid": FieldType.STRING,
}


def postgres_to_field_type(pg_type: str) -> FieldType:
    """Map one Postgres type name to a dtex :class:`~dtex.types.FieldType`.
...
def introspect_schema(
    ...
    :class:`~dtex.types.FieldMode` (``NO`` → ``REQUIRED``, anything else →
    ``NULLABLE``).
