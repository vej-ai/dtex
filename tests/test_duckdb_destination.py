...
Exercises ``dtex/destinations/duckdb/`` hook by hook, without an engine:
...
:class:`~dtex.registry.ConnectorRegistry` the connector folder's decorators
...
def test_ensure_schema_creates_table_with_synced_at(
    ...
    """ensure_schema creates the table and appends _dtex_synced_at."""
...
def test_write_batch_stamps_synced_at(
    ...
    """write_batch fills _dtex_synced_at when a record lacks it — docs/03 §2.2.1."""
    hooks = _hooks(duckdb_destination)
    conn = _open(duckdb_destination, duckdb_path)
    hooks["ensure_schema"](conn, _events_meta())
...
def test_state_table_has_eight_canonical_columns(
    ...
    """_dtex_state has exactly the 8 columns of StateRecord.to_row()."""
    hooks = _hooks(duckdb_destination)
    conn = _open(duckdb_destination, duckdb_path)
...
# --------------------------------------------------------------------------
# write_run_record — _dtex_runs audit table — docs/09 §4
# --------------------------------------------------------------------------
...
def test_write_run_record_creates_table_and_inserts_row(
    ...
    """write_run_record lazily creates _dtex_runs and writes one row per RunRecord."""
    hooks = _hooks(duckdb_destination)
    conn = _open(duckdb_destination, duckdb_path)
    streams = (
    ...
    rows = query_duckdb(
        ...
        "rows_loaded, full_refresh, duration_s, error_type FROM _dtex_runs",
    )
    assert len(rows) == 1
    row = rows[0]
...
def test_write_run_record_persists_streams_json(
    ...
    name = query_duckdb(
        duckdb_path, "SELECT streams_json->>'$[0].name' FROM _dtex_runs"
    )[0][0]
    assert name == "items"
...
def test_write_run_record_is_idempotent_on_run_id(
    ...
    rows = query_duckdb(duckdb_path, "SELECT count(*), max(rows_loaded) FROM _dtex_runs")
    assert rows[0] == (1, 99)
...
def test_write_run_record_persists_error_fields_on_failure(
    ...
    row = query_duckdb(
        duckdb_path,
        "SELECT status, error_type, error_message FROM _dtex_runs",
    )[0]
