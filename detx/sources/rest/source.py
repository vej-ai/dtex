...
1. Declare it under ``streams:`` in ``register.yaml`` (name / table /
   ...
   only what the detx contract supports.
2. Write a ``@stream(name="X")`` function here that calls
   ...
   live in ``register.yaml`` because :class:`~detx.types.ParamSpec` is
...
# contract type :class:`~detx.types.ParamSpec` only accepts scalar values,
...
from detx.sources.rest.pagination import build_strategy
...
def extract_stream(
    *,
    ...
    4. For incremental streams, sends the cursor's
       :meth:`~detx.types.Cursor.start_value` as
