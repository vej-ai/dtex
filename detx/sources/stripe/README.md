...
land as `JSON` columns — the detx convention for nested data
(docs/04 §"Nested data").
...
of arrival order. Per detx's commit model the cursor is committed only
...
`default_destination`. Running it is the standard detx one-liner:
...
```sh
detx run stripe --target prod --select charges
```
