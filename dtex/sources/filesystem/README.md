...
A baked dtex source that reads flat files (CSV, JSONL, Parquet) from a
...
record as `_dtex_file_cursor` (a synthetic field the source attaches), so
the engine's standard `Cursor` + `_dtex_state` machinery just works.
...
| `gs://`   | `GcsBackend`     | `pip install 'dtex[gcs]'`                   |
| `s3://`   | `S3Backend`      | `pip install 'dtex[s3]'`                    |
...
`_dtex_file_cursor` (STRING)** in it (the contract requires any declared
schema to contain the cursor field, see `dtex/types.py::StreamDef`).
