...
extra (``simple-e[gcs]`` / ``simple-e[s3]``) so the fix is one line.
...
class FileRef:
    ...
    * :attr:`cursor_key` — a human-readable string the source attaches to
      every record as the synthetic ``_simple_e_file_cursor`` field. For
...
def _lazy_import_gcs() -> Any:
    ...
    The error message names the install extra (``simple-e[gcs]``) so a user
    ...
    try:
        ...
        raise ImportError(
            ...
            "install with `pip install simple-e[gcs]`"
        ) from exc
    return gcs_storage
...
def _lazy_import_boto3() -> Any:
    ...
    try:
        ...
        raise ImportError(
            ...
            "install with `pip install simple-e[s3]`"
        ) from exc
    return boto3
