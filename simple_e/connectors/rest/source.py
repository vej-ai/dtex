...
1. Declare it under ``streams:`` in ``register.yaml`` (name / table /
   ...
   only what the simpl.E contract supports.
2. Write a ``@stream(name="X")`` function here that calls
   ...
   live in ``register.yaml`` because :class:`~simple_e.types.ParamSpec` is
...
# contract type :class:`~simple_e.types.ParamSpec` only accepts scalar values,
...
from simple_e.connectors.rest.extractors import extract_records
from simple_e.connectors.rest.pagination import build_strategy
...
def extract_stream(
    *,
    ...
    4. For incremental streams, sends the cursor's
       :meth:`~simple_e.types.Cursor.start_value` as
