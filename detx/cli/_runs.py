...
same way :mod:`detx.cli._state` does for ``detx state list``.
...
# ``_detx_runs`` alongside ``_detx_state``). Different configs in one project
...
#
#   ``detx runs list -p <config>`` (config name disambiguates the destination)
#
# This mirrors ``detx state list`` exactly and avoids inventing a multi-
...
# NOTE: like :mod:`detx.cli._state`, this module reaches past the destination
...
class RunRow:
    """One row from ``_detx_runs`` — the shape the CLI prints + admin reads."""
...
def _resolve(
    ...
    *,
    ...
    The same stage-1-through-3 path as :func:`detx.run`, stopping once the
...
def _row_to_runrow(row: tuple[Any, ...]) -> RunRow:
    """Build a :class:`RunRow` from a ``_detx_runs`` row in column order.
...
def _query_runs(conn: Any, *, run_id: str | None, limit: int | None) -> list[RunRow]:
    """SELECT ``_detx_runs`` rows, optionally filtered to a single ``run_id``."""
    ...
    try:
        if Capability.RUN_RECORDS not in dest.capabilities:
            raise StateError(
                ...
                f"Capability.RUN_RECORDS; no _detx_runs table is written"
            )
...
def get_run(
    ...
    *,
    ...
    log_path = dest.project_root / ".detx" / "logs" / run_id / "run.jsonl"
