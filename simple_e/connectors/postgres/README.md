...
A simpl.E source connector that extracts rows from a Postgres database. Each
...
`simple_e/connectors/postgres/`. A user project can shadow it by dropping a
...
`schema:`) maps to one of simpl.E's `FieldType` members (docs/03 §2.2.1):
...
`application_name` (default `simple_e`), `connect_timeout_seconds` (default
