...
* **Where is the project?** :func:`find_project_root` walks up from a starting
  directory for ``simple_e_project.yml`` (docs/02, docs/06).
* **What does a connector NAME resolve to?** :func:`resolve_connector` turns a
  ...
  :class:`~simple_e.registry.ConnectorRegistry` the connector's decorators
  populated. Resolution order is fixed (docs/03 §5): project-local
  ``connectors/<name>/`` wins over the baked ``simple_e/connectors/`` (sources)
  / ``simple_e/destinations/`` (destinations).

The connector-folder *import* mechanism — open a
:func:`~simple_e.registry.registration_scope`, exec every ``.py`` file under a
...
PROJECT_FILE = "simple_e_project.yml"
...
class DiscoveryError(Exception):
    ...
    runner converts it into a ``FAILED`` :class:`~simple_e.types.RunResult`
...
class LoadedConnector:
    ...
    ``register.yaml`` (:class:`~simple_e.types.ConnectorManifest`) and the
    :class:`~simple_e.registry.ConnectorRegistry` the connector's decorators
...
def find_project_root(start: Path | str | None = None) -> Path:
    """Walk up from ``start`` to the directory holding ``simple_e_project.yml``.

    docs/02: "find the project root (walk up for ``simple_e_project.yml``)".
    ...
    raise DiscoveryError(
        ...
        f"a simpl.E project must contain a {PROJECT_FILE} (docs/06)"
    )
...
def _baked_dirs() -> tuple[Path, Path]:
    """Return the two baked connector roots shipped inside the ``simple_e`` package.

    docs/03 §5 / docs/06: baked *sources* live under ``simple_e/connectors/``
    and baked *destinations* under ``simple_e/destinations/``. Both are searched;
...
def find_connector_folder(
    ...
    2. The baked roots (``simple_e/connectors/``, ``simple_e/destinations/``).
...
def _import_module_from_path(path: Path) -> None:
    ...
    """
    unique_name = f"_simple_e_connector_{path.stem}_{uuid.uuid4().hex}"
...
def _load_connector_folder(folder: Path) -> LoadedConnector:
    ...
    :func:`~simple_e.registry.registration_scope`, so every ``@stream`` /
