...
dtex binds `streams[].name` in `register.yaml` to a `@stream(name=...)`
...
The YAML carries only what the dtex contract supports (name / table /
...
The dtex contract type `ParamSpec` (docs/03 §2.4) only allows scalar
