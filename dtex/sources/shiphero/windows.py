...
* :func:`compute_start` — apply the ``lookback_days`` subtraction to the cursor
  resume value. The dtex engine deliberately does NOT do this (see the
...
# implementation in ``dtex/engine/runner.py::_seed_value`` returns the
...
def to_utc_dt(value: datetime | str | None) -> datetime | None:
    ...
    The dtex engine seeds the cursor with one of two shapes depending on
