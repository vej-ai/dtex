# BigQuery destination — dtex v1

The production warehouse dtex was always meant to land in. Tier A: BigQuery
hosts its own state and audit tables (`_dtex_state`, `_dtex_runs`) alongside
...
`google-cloud-storage`, and `pyarrow` ship in dtex's base dependencies.
...
2. **Application Default Credentials (ADC)** — leave `credentials_path`
   empty (the default). dtex then picks up `GOOGLE_APPLICATION_CREDENTIALS`,
...
dtex never logs credentials. Service-account paths may appear in error
...
bigquery:
  ...
  targets:
    prod:
      ...
      staging_bucket: my-dtex-staging   # REQUIRED (you create it; dtex never does)
      location: US                     # default "US"
      staging_prefix: dtex/staging      # default
...
(dtex cleans up on success; on failure it leaves the Parquet for forensics).
...
BigQuery has no general `BEGIN` / `COMMIT` spanning multiple jobs. dtex v1
...
`dtex state reset -p <config>`; tomorrow: `dtex state reset --recreate-table`,
...
- `_dtex_state` — one row per `(connector, stream)` resume point. Eight
...
- `_dtex_runs` — one row per run; `run_id` is the upsert key.
