...
(:func:`resolve_secret_url`), the project-local ``detx_plugins.py``
...
package adds itself with a tiny ``[project.entry-points."detx.secret_resolvers"]``
block — no engine change.

# NOTE: the existing ``${env.X}`` / ``${profile.X.Y}`` resolution lives in
# :mod:`detx.engine.config` and stays unchanged. Those are string-interpolation
...
Public API (re-exported from :mod:`detx`):
...
* :func:`resolve_secret_url` — the parser + dispatch used by
  :mod:`detx.engine.config`.
* :func:`is_secret_url` — the dispatch predicate.
* :func:`load_project_plugins` — the project-local ``detx_plugins.py``
  importer (called once per project from :mod:`detx.engine.discovery`).
...
from detx.secrets.resolvers import (
