...
production warehouse dtex was always meant to land in — built to the same
contract as the DuckDB reference (see ``dtex/destinations/duckdb/``), with
...
# ``dtex/engine/runner.py::_stream_transaction``). A future opt-in
...
from dtex.destinations.bigquery import client as _client_mod
from dtex.destinations.bigquery.client import (
    BigQueryClient,
    BQConn,
    build_client,
    run_with_retries,
...
# exception class, not the engine's :class:`~dtex.engine.runner.EngineError`.
# The task spec asks for an EngineError on partition drift, but a
# destination importing from ``dtex.engine.runner`` would introduce a reverse
...
# folds it into a ``FAILED`` :class:`~dtex.types.RunResult`, so a
...
class PartitionDriftError(RuntimeError):
    ...
    :class:`~dtex.types.PartitionConfig` requests — see :func:`compare_partition`
...
# ``dtex.destinations.bigquery.client`` module. Going through the attribute
...
# The engine-owned state and runs tables — same ``_dtex_`` namespace as the
...
def capabilities() -> set[Capability]:
    ...
    * ``STATE`` — BigQuery hosts the ``_dtex_state`` table itself (Tier A,
    ...
    * ``RUN_RECORDS`` — BigQuery hosts the ``_dtex_runs`` audit table
      alongside ``_dtex_state``, queryable with plain SQL (docs/09 §4).
...
def max_concurrent_writes(config: Config) -> int:
    ...
    ``dtex run --tag X --threads 4`` against five BigQuery configs caps at
...
def open(config: Config) -> BQConn:  # noqa: A001 — hook name is fixed by the contract
    ...
    * ``job_timeout_seconds`` / ``retry_max_attempts`` / ``retry_backoff_seconds``
    ...
    — dtex never creates buckets, that is an operator concern (region,
...
def _verify_staging_bucket(client: BigQueryClient) -> None:
    ...
    dtex does not create the staging bucket: region / retention / lifecycle
...
def ensure_schema(conn: BQConn, stream: StreamMeta) -> None:
    ...
    The engine appends ``_dtex_synced_at`` to every record (docs/03 §2.2.1);
...
def _arrow_type_for(field_type: FieldType, pa: Any) -> Any:
    """Map a dtex :class:`FieldType` to a pyarrow type — the Parquet column type."""
    ...
    if field_type is FieldType.TIMESTAMP:
        ...
        # engine stamps ``_dtex_synced_at`` in UTC; aware datetimes carry the
...
def _merge_via_staging(
    ...
    :func:`~dtex.destinations.bigquery.ddl.merge_sql` and contains only
...
def _state_schema() -> Iterator[tuple[str, FieldType]]:
    """The canonical ``_dtex_state`` column set — mirrors ``StateRecord.to_row()``.
...
# --------------------------------------------------------------------------
# write_run_record — the _dtex_runs audit table — docs/09 §4
# --------------------------------------------------------------------------


def _runs_schema() -> Iterator[tuple[str, FieldType, str]]:
    """The canonical ``_dtex_runs`` column set — mirrors the DuckDB destination.
...
def _lease_schema() -> Iterator[tuple[str, FieldType]]:
    """The canonical ``_dtex_leases`` column set — mirrors LeaseRecord.to_row()."""
    yield "connector", FieldType.STRING
    yield "stream", FieldType.STRING
    yield "run_id", FieldType.STRING
    yield "status", FieldType.STRING
    yield "acquired_at", FieldType.TIMESTAMP
    yield "heartbeat_at", FieldType.TIMESTAMP
...
def read_leases(conn: BQConn, connector: str) -> list[LeaseRecord]:
    """Load every ``_dtex_leases`` row for a connector — docs/05 §5.5.

    Liveness is judged by the engine (:meth:`LeaseRecord.is_live`); this hook
    returns raw rows via a parameterized query.
    """
    ...
    sql = (
        ...
        f"FROM {table} WHERE connector = @connector"
    )
    job_config = bq.QueryJobConfig(
        query_parameters=[bq.ScalarQueryParameter("connector", "STRING", connector)]
    )
...
def acquire_lease(conn: BQConn, lease: LeaseRecord) -> bool:
    ...
    sql = (
        ...
        f"  OR TIMESTAMP_DIFF(S.heartbeat_at, T.heartbeat_at, SECOND) > @stale "
        f") THEN UPDATE SET "
        f"  run_id = S.run_id, status = S.status, "
        f"  acquired_at = S.acquired_at, heartbeat_at = S.heartbeat_at "
        f"WHEN NOT MATCHED THEN INSERT "
    ...
    rows = list(
        client.bq.query(check, job_config=check_config, location=client.location).result(
            timeout=client.job_timeout_seconds
        )
    )
