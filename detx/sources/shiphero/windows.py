...
* :func:`compute_start` — apply the ``lookback_days`` subtraction to the cursor
  resume value. The detx engine deliberately does NOT do this (see the
...
# implementation in ``detx/engine/runner.py::_seed_value`` returns the
...
def to_utc_dt(value: datetime | str | None) -> datetime | None:
    ...
    The detx engine seeds the cursor with one of two shapes depending on
