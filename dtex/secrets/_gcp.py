...
(see :mod:`dtex.destinations.bigquery.client`).
...
# run's :class:`~dtex.engine.logger.Redactor`. The engine does that
# post-hoc in :mod:`dtex.engine.runner` (see
...
# 429 with exponential backoff; layering dtex-side retries would double
...
from dtex.secrets.resolvers import SecretResolutionError

logger = logging.getLogger("dtex.secrets")
...
def _lazy_import_gcp_secrets() -> Any:
    ...
    :func:`dtex.sources.filesystem.backends._lazy_import_gcs` and
    :func:`dtex.destinations.bigquery.client._bigquery_module` — the SDK is
    NOT imported at ``dtex`` package import time, so a base install (or one
...
class GcpSecretManagerResolver:
    ...
    Implements the :class:`~dtex.secrets.SecretResolver` Protocol. The
    ...
    * ``__init__`` does NOT import the SDK or build any client. The
      ...
      :func:`~dtex.secrets.resolvers._get_resolver` on first URL referencing
    ...
    * The cache is per-instance, and the instance is itself cached
      ...
      SDK client per ``dtex`` process, however many ``secret://gcp-secret-manager/...``
      references the run consumes.

    Errors:

    * Malformed path (does not match :data:`_RESOURCE_NAME`) →
      :class:`~dtex.secrets.SecretResolutionError` with the validation
      message.
    * ``google.api_core.exceptions.PermissionDenied`` →
      :class:`~dtex.secrets.SecretResolutionError` with "permission denied
    ...
    * ``google.api_core.exceptions.NotFound`` →
      :class:`~dtex.secrets.SecretResolutionError` with "secret X version
      Y not found".
    * Any other SDK error → :class:`~dtex.secrets.SecretResolutionError`
    ...
    def resolve(self, path: str, field: str | None) -> str:
        ...
        Returns:
            ...
            :class:`~dtex.secrets.SecretResolutionError` (the underlying
            :class:`UnicodeDecodeError` is chained).

        Raises:
            :class:`~dtex.secrets.SecretResolutionError` — see the class
        ...
        try:
            ...
            raise SecretResolutionError(
                ...
                f"dtex requires resolved secret values to be strings"
            ) from exc
