...
* ``path`` — opaque to detx, handed to the resolver verbatim. Required to be
...
* **Entry-points** — third-party packages register a resolver factory under the
  ``detx.secret_resolvers`` group. Loaded once on first
  :func:`resolve_secret_url` call (lazy — a project that uses zero
  ``secret://`` refs never pays the import cost).
* **Project-local ``detx_plugins.py``** — a Python file at the project root
  (sibling of ``detx_project.yml``). Imported once per project, at startup,
...
1. Project-local ``detx_plugins.py`` registrations.
...
# writing ``register_secret_resolver("gcp", ...)`` in ``detx_plugins.py`` has
...
* "Fresh-every-run" (docs/08 Q11) is the per-process commitment: no on-disk
  cache. A new ``detx run`` invocation is a new process; it pays the resolver
...
# (per-import-call mutables), but in normal use detx runs as one process per
# CLI invocation.
...
def register_secret_resolver(
    ...
    Called from a project's ``detx_plugins.py``, or from third-party packages
    ...
    Raises:
        ...
        ``detx_plugins.py`` execution order, which is the kind of bug whose
...
def is_secret_url(value: Any) -> bool:
    ...
    Used by the config layer (:func:`detx.engine.config.resolve_secret_ref`)
...
_ENTRY_POINT_GROUP = "detx.secret_resolvers"


def _load_entry_points() -> None:
    """Discover entry-points under ``detx.secret_resolvers``; register each
    ...
    # Project-local ``detx_plugins.py`` runs at startup (BEFORE the first
    ...
    try:
        ...
        # ``detx_plugins.py`` should still work.
...
# Track project roots whose ``detx_plugins.py`` has already been imported, so
# a re-call (multiple ``detx.run(...)`` invocations against the same project
...
def load_project_plugins(project_root: Path) -> None:
    """Import ``<project_root>/detx_plugins.py`` if present — runs its
    registration calls.
    ...
    ``destinations/<name>/destination.py``) — detx imports project Python
    by design.

    Args:
        project_root: the directory holding ``detx_project.yml`` (the value
            :func:`detx.engine.discovery.find_project_root` returns).
    ...
    # import harness in :mod:`detx.engine.discovery`. In practice ``detx
    # run`` is one process per invocation; the harness exists for the
    # programmatic ``detx.run(...)`` looping case.
    """
    plugin_path = project_root / "detx_plugins.py"
    with _PROJECT_PLUGINS_LOCK:
        ...
        # harness (``_detx_connector_<stem>_<uuid>``). Uses a stable name
        # keyed by the project path so two calls for the same project
        # don't pollute sys.modules with N copies.
        module_name = f"_detx_plugins_{abs(hash(str(project_root)))}"
...
def resolve_secret_url(url: str) -> str:
    ...
    :func:`detx.engine.config.resolve_secret_ref` when a ``register.yaml``
    ...
    # returned string to the run's :class:`~detx.engine.logger.Redactor` so
...
def _reset_resolvers_for_testing() -> None:
    ...
    # test (the test writes a detx_plugins.py to a tmp_path project; without
