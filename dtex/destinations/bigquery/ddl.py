...
* the dtex :class:`~dtex.types.FieldType` → BigQuery native-type mapping
  (docs/05 §3.1);
* the :class:`~dtex.types.FieldMode` → BigQuery mode mapping;
...
The mapping is total over :class:`~dtex.types.FieldType`'s eight members and
:class:`~dtex.types.FieldMode`'s three members.
"""
...
"""dtex :class:`FieldType` → BigQuery column type — docs/05 §3.1.
...
def bigquery_type(field_type: FieldType) -> str:
    """Return the BigQuery native type for a dtex :class:`FieldType` — docs/05 §3.1.
...
def bigquery_mode(field_mode: FieldMode) -> str:
    """Return the BigQuery field mode for a dtex :class:`FieldMode`.
...
# tables/columns (``_dtex_state``, ``_dtex_synced_at``) start with one.
...
def validate_identifier(name: str, *, kind: str = "identifier") -> str:
    ...
    dtex table, column and dataset names come from ``register.yaml`` and from
...
def bq_schema_field(field: Field) -> Any:
    """Build a ``bigquery.SchemaField`` for one declared dtex :class:`Field`.

    The BigQuery SDK is imported lazily because the ``bigquery`` extra may
    not be installed in a base ``dtex`` environment (the rest of the package
...
def bq_schema(schema: Schema) -> list[Any]:
    """Build the ``list[bigquery.SchemaField]`` for a dtex :class:`Schema`.

    The order matches ``schema.fields`` — ``_dtex_synced_at`` (when present)
...
# --------------------------------------------------------------------------
# Partitioning — convert dtex's PartitionConfig to BigQuery SDK objects
# and compare against an existing table's partitioning. docs/05 §3.x.
# --------------------------------------------------------------------------

# Maps dtex's TimeGranularity to BigQuery's TimePartitioningType string. The
...
def apply_partitioning_to_table(
    ...
    Called by ``ensure_schema`` on the table-creation path. Maps a dtex
...
def existing_table_partition(table: Any) -> PartitionConfig | None:
    """Normalize an existing ``bigquery.Table``'s partition spec to dtex shape.
...
def compare_partition(
    ...
    # NOTE: ``dtex state reset --recreate-table`` is the user-facing path the
    ...
    # ``dtex state reset -p <config>`` to clear the state row, then re-run.
    """
    ...
    msg = (
        ...
        f"table's partitioning in place. To resolve: either (a) run `dtex state "
