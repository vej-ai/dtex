...
* Each decorator *also* stamps its metadata onto the wrapped function object
  (``__detx_*__`` attributes). The function therefore *self-describes*: a
...
* Harvesting ``__detx_*__`` attributes by walking every module after import
...
_ATTR_KIND = "__detx_kind__"
_ATTR_STREAM_NAME = "__detx_stream_name__"
_ATTR_INJECT = "__detx_inject__"
_ATTR_HOOK = "__detx_hook__"
_ATTR_IS_METHOD = "__detx_is_method__"
...
def compute_injection(
    ...
    if declared is None:
        raise TypeError(
            f"{getattr(func, '__name__', func)!r} is not a detx stream function; "
...
class StreamRegistration:
    ...
    (a :class:`~detx.types.StreamDef`) to its implementing function
...
with a fully-built :class:`~detx.types.RunRecord`. It is the destination's
half of the run-record audit table (``_detx_runs``); the per-run JSONL log
...
the engine reads it (with the resolved destination :class:`~detx.types.Config`)
and clamps the number of pipelines that may concurrently target this
destination under ``detx run --tag … --threads N``. Returning ``1`` (DuckDB)
...
The hook is NOT tied to a :class:`~detx.types.Capability` flag — every
...
def stream(name: str) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
    ...
    * returns the function **unchanged and directly callable**, with
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        wrapper.__detx_kind__ = ConnectorKind.SOURCE  # type: ignore[attr-defined]
        wrapper.__detx_stream_name__ = name  # type: ignore[attr-defined]
        wrapper.__detx_inject__ = inject  # type: ignore[attr-defined]
        wrapper.__detx_is_method__ = False  # type: ignore[attr-defined]
...
class _DestinationNamespace:
    ...
    *namespace* of hooks rather than one function. ``destination`` is the single
    ...
    def __repr__(self) -> str:
        ...
        return f"<detx.destination namespace: {', '.join(sorted(self._hooks))}>"
...
def _make_hook_decorator(
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        wrapper.__detx_kind__ = ConnectorKind.DESTINATION  # type: ignore[attr-defined]
        wrapper.__detx_hook__ = hook  # type: ignore[attr-defined]
...
def stream_method(
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        wrapper.__detx_stream_name__ = name  # type: ignore[attr-defined]
        wrapper.__detx_inject__ = inject  # type: ignore[attr-defined]
        wrapper.__detx_is_method__ = True  # type: ignore[attr-defined]
...
class Connector:
    ...
    * :attr:`config` — the resolved :class:`~detx.types.Config`, the same
    ...
    * ``@stream_method(name=...)`` — marks an instance method as a stream.
    ...
    config: Any
    """The resolved :class:`~detx.types.Config`. Assigned by the engine before :meth:`setup`."""
