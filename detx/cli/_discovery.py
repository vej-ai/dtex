...
component" call; the engine's :mod:`detx.engine.discovery` resolves only *by
name*, and :mod:`detx.engine.configs` only loads configs. This module supplies
...
parses each ``register.yaml`` into a :class:`~detx.types.ConnectorManifest`,
and yields the result. It is **not** engine logic — no data ever moves here.

For configs: :func:`discover_all_configs` re-uses
:func:`detx.engine.configs.discover_configs` directly.
"""
...
from detx.types import ConnectorKind, ConnectorManifest, PipelineConfig
...
class DiscoveredConnector:
    ...
    destination paths and ``"baked"`` for one shipped inside the ``detx``
    package, so ``detx list`` can show where a connector came from.
    """
...
def discover_all_sources(
    ...
    then the baked ``detx/sources/``. Project-local shadows baked. Folders
...
def discover_all_destinations(
    ...
    the baked ``detx/destinations/``. Folders whose ``register.yaml`` declares
...
def discover_all_configs(
    ...
    Delegates to :func:`detx.engine.configs.discover_configs` and returns the
    parsed :class:`PipelineConfig` objects in sorted-name order, ready for
    ``detx list --kind config`` rendering.
    """
    return sorted(
