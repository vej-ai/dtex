...
# ``{config, state, cursor, log}`` (``dtex.registry.STREAM_INJECTABLES``).
...
from dtex.sources.shiphero.client import ShipHeroClient
from dtex.sources.shiphero.pagination import paginate
from dtex.sources.shiphero.queries import (
    ...
    for win_from, win_to in date_windows(start, step_days=int(config.step_days)):
        ...
        for raw_record in paginate(
            ...
            if value is not None:
                ...
                # destination's ``_dtex_state.cursor_value`` is a ``JSON``
