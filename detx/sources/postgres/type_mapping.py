...
# runtime hook. detx's contract gives a ``@stream`` function no way to hand
...
from detx import Field, FieldMode, FieldType, Schema

if TYPE_CHECKING:
    import psycopg

# The complete, fixed mapping from a normalised Postgres type name to a
# detx :class:`~detx.types.FieldType`. Keys are the lower-cased
...
_PG_TO_FIELD_TYPE: dict[str, FieldType] = {
    ...
    # uuid → STRING per the task spec (no native UUID field type in detx)
    "uuid": FieldType.STRING,
}


def postgres_to_field_type(pg_type: str) -> FieldType:
    """Map one Postgres type name to a detx :class:`~detx.types.FieldType`.
...
def introspect_schema(
    ...
    :class:`~detx.types.FieldMode` (``NO`` → ``REQUIRED``, anything else →
    ``NULLABLE``).
