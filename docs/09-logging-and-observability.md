...
The operator's requirement is plain: **"I want to know exactly what is happening."** simpl.E meets it with one mechanism — structured, per-run logging — used two ways: a human-readable stream while a run is in progress, and a machine-readable record after it. There is no separate metrics system, no agent, no telemetry. Logs *are* the observability layer.
...
- **Self-hosted, no phone-home.** Logs are written to the local filesystem and (optionally) the destination. simpl.E sends nothing anywhere.
...
While `simple-e run` blocks (it always runs synchronously — see [07 §3](./07-cli-and-library-api.md)), it streams events to stdout:
...
Independent of stdout verbosity, **every run writes a complete `debug`-level JSON-lines file** under the project's `.simple_e/` directory:
...
Log retention is the operator's call: `.simple_e/logs/` is gitignored and can be cleaned by cron or by `simple-e state` housekeeping. simpl.E does not auto-delete logs in v1.

> [Open question: a `simple-e logs <run_id>` command to pretty-print a past run's `run.jsonl`, and a `--keep-logs <n>` retention flag. Both are small, both are quality-of-life, neither is v1-critical. Proposal: v2.]
...
Run records follow the **same capability-tier model as state** ([05 §5.4](./05-destinations-and-state.md)) — this keeps simpl.E's data-placement story consistent: data, state, and run records all sit together, wherever "together" is.

- **Tier A destinations** → a `_simple_e_runs` table in the destination, alongside `_simple_e_state`. One row per run. This makes run history *queryable with SQL* — "show me every failed stripe run this week", "rows loaded per day" — with no extra tooling. The table:
...
- **Tier B destinations** (object storage, no tables) → the run record is written as `.simple_e/runs/<run_id>.json` locally, and optionally mirrored as a `_simple_e_runs/<run_id>.json` object in the bucket. Symmetric with the Tier B sidecar-state design.

The run record is **always** written locally to `.simple_e/runs/` regardless of tier, so run history survives even if the destination is unreachable for the write. The destination copy is the shared, queryable source of truth; the local copy is the durable fallback.

A run record is written even when a run **fails** — `status: "failed"` with a populated `error`. A failed run that produced no run record means simpl.E itself crashed before it could write one; that is the only ambiguous state, and the per-run log file (written incrementally) covers it.
...
**Redaction is enforced in the logging layer**, per the security contract in [08 §6](./08-security.md): any value marked `secret: true`, or resolved from `${ENV_VAR}` / `secret://`, is replaced with `***` in *every* sink — stdout, `run.jsonl`, and the `_simple_e_runs` record. Redaction is value-based: simpl.E scrubs known secret values out of log strings before writing, so a secret echoed inside an HTTP error body is also caught. This is best-effort against a hostile connector ([08 §7](./08-security.md)) but a reliable guard against accidental leakage.
...
- **A future UI** reads `_simple_e_runs` and `_simple_e_state` straight from the destination, plus `run.jsonl` for run drill-down. No new collection layer, no agent — the UI is a *reader* of data simpl.E already writes.
...
- **Alerting** is just a query: a scheduled check over `_simple_e_runs` for `status = 'failed'`, or a CI step that inspects the exit code ([07 §3](./07-cli-and-library-api.md)).
...
- State lifecycle, capability tiers, `.simple_e/` layout → [05 — Destinations and State](./05-destinations-and-state.md)
