...
(:mod:`.readers`), and yields :class:`~dtex.Batch` of records — with the
synthetic ``_dtex_file_cursor`` field attached to every record.
...
extra field (``_dtex_file_cursor``) on every record the source yields:

* the engine sees a normal cursor field — every existing mechanism
  (``Cursor.observe``, ``_dtex_state``, ``cursor.start_value`` filtering
...
# The synthetic record field (``_dtex_file_cursor``) is always the
...
# ``_dtex_state.cursor_value``) is the ``cursor_observe`` value: a
...
# (see dtex/connectors/shiphero/source.py lines 154-165). This bug
...
from dtex.sources.filesystem.backends import FileRef, pick_backend
from dtex.sources.filesystem.readers import infer_format, reader_for_format
...
def files(config: Config, cursor: Cursor) -> Iterator[Batch]:
    ...
    Every yielded record carries ``_dtex_file_cursor`` — the file's
    ...
    ``_dtex_state``, so the next run resumes past every loaded file.
    """
    ...
    # the value comes back from `_dtex_state.cursor_value` (a JSON
