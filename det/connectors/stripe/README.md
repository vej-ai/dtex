...
land as `JSON` columns — the det convention for nested data
(docs/04 §"Nested data").
...
of arrival order. Per det's commit model the cursor is committed only
...
`default_destination`. Running it is the standard det one-liner:
...
```sh
det run stripe --target prod --select charges
```
