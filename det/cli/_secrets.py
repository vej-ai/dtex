"""``det secrets test`` — verify that every declared secret reference resolves.
...
# NOTE: design decision — ``det secrets test`` is the place where we
# DELIBERATELY resolve every reference up front, even though normal
# ``det run`` resolution is lazy. The whole purpose of the command is the
...
# :mod:`det.secrets`). That's the same cost a real run would pay.
"""
...
from det.cli._discovery import discover_all_configs
from det.engine import config as cfg
from det.engine import discovery as disc
from det.secrets import load_project_plugins
from det.types import PipelineConfig, SecretRef


@dataclass(frozen=True)
class ReferenceCheck:
    """One per-reference verification result for ``det secrets test``.
...
def _resolve_one(
    ...
    Wraps :func:`det.engine.config.resolve_secret_ref` so every exception
...
def check_config(
    ...
    *,
    ...
    ``default_target`` (:func:`det.engine.config.resolve_target_name`).
...
def check_project(
    ...
    *,
    ...
    """Drive ``det secrets test`` end to end — returns (checks, ok_count, fail_count).

    * ``project_dir`` — project root, or any dir under it; walked up to
      find ``det_project.yml``.
    ...
    * ``target_override`` — overrides the config's ``target:`` (the
    ...
    The project's ``det_plugins.py`` is imported BEFORE any resolution,
    matching ``det run``'s behavior so a project-local custom
    ...
    # registered in det_plugins.py would resolve as "unknown scheme".
