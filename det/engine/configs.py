...
config; the engine resolves it to a :class:`~det.types.PipelineConfig` and
runs its lifecycle. This module owns:

* **Discovery** — :func:`discover_configs` walks each
  :class:`~det.engine.config.ProjectConfig.config_paths` directory (default:
...
* **Lookup** — :func:`load_config` resolves one config by name through the
  same discovery, raising a clear :class:`~det.engine.config.ConfigError` for
