...
Plain module — no decorators, no simpl.E contract — exactly the "API client"
...
:mod:`simple_e.connectors.stripe.pagination`, kept separate so this file is
...
class StripeClient:
    ...
    builds a client from :class:`~simple_e.types.Config` at the top of the
    stream, then drives the cursor pagination loop in
    :mod:`simple_e.connectors.stripe.pagination` against it. A long-lived
