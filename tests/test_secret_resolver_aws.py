"""Tests for :class:`dtex.secrets._aws.AwsSecretsManagerResolver` — stage 9c.

Two paths:

* **Unit tests** (always run): substitute a fake ``boto3`` module via
  ``monkeypatch.setattr(dtex.secrets._aws, "_lazy_import_boto3", ...)``
...
runner's credential chain can access)::
    ...
    aws secretsmanager create-secret \\
        --name dtex-it-secret \\
        --secret-string 'hello-from-dtex-test' \\
        --region us-east-1
    # then in the shell that runs pytest:
    export DET_AWS_SECRETS_TEST_REGION=us-east-1
    export DET_AWS_SECRETS_TEST_SECRET_ID=dtex-it-secret
"""
...
from dtex.secrets import _aws as aws_resolver_mod
from dtex.secrets._aws import AwsSecretsManagerResolver
...
def test_missing_sdk_raises_import_error_with_install_hint(
    ...
    with pytest.raises(ImportError) as exc_info:
        resolver.resolve("us-east-1/mysecret", None)
    assert "pip install dtex[aws-secrets]" in str(exc_info.value)
...
def test_entry_point_registration_populates_registry() -> None:
    ...
    ``aws-secrets-manager`` under the ``dtex.secret_resolvers`` group.
    """
