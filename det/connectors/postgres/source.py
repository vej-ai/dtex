...
# engine (det/engine/config.py::build_config) constructs a single
# connector-level :class:`~det.types.Config` and injects only that into a
...
the engine injects a :class:`~det.types.Cursor` (docs/03 §3.2). Both
...
def _extract_full_scan(
    *,
    ...
    """
    batch_size = int(config.batch_size)
    cursor_name = f"_det_pg_{uuid.uuid4().hex[:12]}"
