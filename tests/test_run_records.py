...
* the per-run JSON-lines file at ``.det/logs/<run_id>/run.jsonl`` (the
  *narrative*); and
* the destination-side ``_det_runs`` audit table (the *receipt*) written via
...
from det.engine.runner import EngineError
...
def project(tmp_path: Path) -> Path:
    """Copy the fixture project into a temp dir so .det/ writes are isolated."""
...
# ---------------------------------------------------------------------------
# Successful run: _det_runs row + JSONL log
# ---------------------------------------------------------------------------


def test_successful_run_writes_det_runs_row(project: Path, warehouse: str) -> None:
    """A successful run lands one fully-populated row in _det_runs."""
    result = det.run(
        config="echo_dev",
...
def test_successful_run_streams_json_carries_per_stream_detail(
    ...
    raw = _query(warehouse, "SELECT streams_json FROM _det_runs")[0][0]
...
def test_successful_run_writes_jsonl_log_with_expected_events(
    ...
    log_path = project / ".det" / "logs" / result.run_id / "run.jsonl"
...
def test_two_consecutive_runs_produce_two_det_runs_rows(
    project: Path, warehouse: str
) -> None:
    ...
    rows = _query(warehouse, "SELECT count(*) FROM _det_runs")
    assert rows[0][0] == 2
...
def test_failed_run_writes_det_runs_row_with_error(
    project: Path, warehouse: str
) -> None:
    """A failed run still lands one _det_runs row, with FAILED + error_type."""
    ...
    rows = _query(
        warehouse,
        "SELECT run_id, status, error_type, error_message FROM _det_runs",
    )
    assert len(rows) == 1
    row = rows[0]
    assert row[0] == result.run_id
...
def test_failed_run_writes_jsonl_with_stream_failed_event(
    ...
    # The traceback lands in the JSONL (NOT in _det_runs — docs/09 §4 NOTE).
...
def test_destination_without_run_records_capability_still_writes_jsonl(
    ...
    result = det.run(config="echo_tiny", project_dir=str(project))
    assert result.status.value == "succeeded"
...
def test_declaring_run_records_without_hook_fails_engine_error(project: Path) -> None:
    ...
    result = det.run(config="echo_claims", project_dir=str(project))
    assert result.status.value == "failed"
...
# ---------------------------------------------------------------------------
# CLI surface: det runs list / det runs show
# ---------------------------------------------------------------------------
...
def test_cli_runs_list_shows_run_rows(
    ...
    """det runs list -p echo_dev shows the run rows from _det_runs."""
    result = det.run(
        config="echo_dev",
        project_dir=str(project),
        destination_params_override={"path": warehouse},
...
def test_cli_runs_list_empty_message(
    ...
    """det runs list against an empty destination prints a 'no records' message."""
    # Open the destination once to create an empty database, but never run.
    # _det_runs does not yet exist; the query helper treats that as 'no rows'.
...
def test_cli_runs_show_prints_record_and_jsonl(
    ...
    """det runs show <run_id> prints the audit row + every JSONL event."""
    result = det.run(
        config="echo_dev",
        project_dir=str(project),
        destination_params_override={"path": warehouse},
...
def test_write_run_record_failure_is_logged_not_raised(project: Path) -> None:
    ...
    result = det.run(config="echo_breaks", project_dir=str(project))
...
def test_cli_runs_show_accepts_short_id(
    ...
    """det runs show accepts the trimmed (no ``run-`` prefix) id form."""
    result = det.run(
        config="echo_dev",
        project_dir=str(project),
        destination_params_override={"path": warehouse},
