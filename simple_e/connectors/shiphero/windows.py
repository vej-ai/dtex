...
* :func:`compute_start` — apply the ``lookback_days`` subtraction to the cursor
  resume value. The simpl.E engine deliberately does NOT do this (see the
...
# implementation in ``simple_e/engine/runner.py::_seed_value`` returns the
...
def to_utc_dt(value: datetime | str | None) -> datetime | None:
    ...
    The simpl.E engine seeds the cursor with one of two shapes depending on
