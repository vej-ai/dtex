...
"""Human-readable output helpers for the ``dtex`` CLI.

The CLI is a thin shell over the engine; these functions turn the engine's
result objects (:class:`~dtex.types.RunResult`,
:class:`~dtex.types.StateRecord`) and discovery objects into aligned,
...
from dtex.engine import last_run_tag_parallelism
from dtex.types import RunResult, RunStatus, StreamStatus
...
def print_multi_run_summary(tag: str, results: list[RunResult]) -> None:
    """Print a one-table summary of a ``dtex run --tag`` multi-run — stage 8d.
    ...
    # matching :class:`~dtex.types.RunResult.to_dict`. The full traceback
