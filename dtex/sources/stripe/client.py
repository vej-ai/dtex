...
Plain module — no decorators, no dtex contract — exactly the "API client"
...
:mod:`dtex.sources.stripe.pagination`, kept separate so this file is
...
class StripeClient:
    ...
    builds a client from :class:`~dtex.types.Config` at the top of the
    stream, then drives the cursor pagination loop in
    :mod:`dtex.sources.stripe.pagination` against it. A long-lived
