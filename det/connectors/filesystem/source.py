...
(:mod:`.readers`), and yields :class:`~det.Batch` of records — with the
synthetic ``_det_file_cursor`` field attached to every record.
...
extra field (``_det_file_cursor``) on every record the source yields:

* the engine sees a normal cursor field — every existing mechanism
  (``Cursor.observe``, ``_det_state``, ``cursor.start_value`` filtering
...
# The synthetic record field (``_det_file_cursor``) is always the
...
# ``_det_state.cursor_value``) is the ``cursor_observe`` value: a
...
# (see det/connectors/shiphero/source.py lines 154-165). This bug
...
FILE_CURSOR_FIELD = "_det_file_cursor"
...
def files(config: Config, cursor: Cursor) -> Iterator[Batch]:
    ...
    Every yielded record carries ``_det_file_cursor`` — the file's
    ...
    ``_det_state``, so the next run resumes past every loaded file.
    """
    ...
    # the value comes back from `_det_state.cursor_value` (a JSON
