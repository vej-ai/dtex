...
* the dtex :class:`~dtex.types.FieldType` → DuckDB native-type mapping
  (docs/05 §3.1);
...
The mapping is total over :class:`~dtex.types.FieldType`'s eight members.
"""
...
from dtex.types import FieldType, Schema
...
"""dtex :class:`FieldType` → DuckDB column type — docs/05 §3.1.
...
def duckdb_type(field_type: FieldType) -> str:
    """Return the DuckDB native type for a dtex :class:`FieldType` — docs/05 §3.1.
...
# (``_dtex_state``, ``_dtex_synced_at``) start with one.
...
def validate_identifier(name: str, *, kind: str = "identifier") -> str:
    ...
    dtex table and column names come from ``register.yaml`` and from source
