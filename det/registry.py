"""det connector registry — the decorator + registration layer.

This module (``det/registry.py``) is the **author-facing decorator API**
...
* Each decorator *also* stamps its metadata onto the wrapped function object
  (``__det_*__`` attributes). The function therefore *self-describes*: a
...
* Harvesting ``__det_*__`` attributes by walking every module after import
...
from det.types import ConnectorKind
...
_ATTR_KIND = "__det_kind__"
_ATTR_STREAM_NAME = "__det_stream_name__"
_ATTR_INJECT = "__det_inject__"
_ATTR_HOOK = "__det_hook__"
_ATTR_IS_METHOD = "__det_is_method__"
...
def compute_injection(
    ...
    if declared is None:
        raise TypeError(
            f"{getattr(func, '__name__', func)!r} is not a det stream function; "
...
class StreamRegistration:
    ...
    (a :class:`~det.types.StreamDef`) to its implementing function
...
def stream(name: str) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
    ...
    * returns the function **unchanged and directly callable**, with
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        wrapper.__det_kind__ = ConnectorKind.SOURCE  # type: ignore[attr-defined]
        wrapper.__det_stream_name__ = name  # type: ignore[attr-defined]
        wrapper.__det_inject__ = inject  # type: ignore[attr-defined]
        wrapper.__det_is_method__ = False  # type: ignore[attr-defined]
...
class _DestinationNamespace:
    ...
    *namespace* of hooks rather than one function. ``destination`` is the single
    ...
    def __repr__(self) -> str:
        ...
        return f"<det.destination namespace: {', '.join(sorted(self._hooks))}>"
...
def _make_hook_decorator(
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        wrapper.__det_kind__ = ConnectorKind.DESTINATION  # type: ignore[attr-defined]
        wrapper.__det_hook__ = hook  # type: ignore[attr-defined]
...
def stream_method(
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        wrapper.__det_stream_name__ = name  # type: ignore[attr-defined]
        wrapper.__det_inject__ = inject  # type: ignore[attr-defined]
        wrapper.__det_is_method__ = True  # type: ignore[attr-defined]
...
class Connector:
    ...
    * :attr:`config` — the resolved :class:`~det.types.Config`, the same
    ...
    * ``@stream_method(name=...)`` — marks an instance method as a stream.
    ...
    config: Any
    """The resolved :class:`~det.types.Config`. Assigned by the engine before :meth:`setup`."""
