"""Project + connector scaffolding for ``simple-e init`` / ``simple-e new``.

These functions write the starter files docs/06 describes — a project tree
(``simple_e_project.yml``, ``profiles.yml``, ``connectors/``, ``destinations/``,
...
_PROJECT_YML = """\
# {name} - a simpl.E project (docs/06).
#
...
# selected by `simple-e run --target <name>`.
targets:
  ...
  dev:
    ...
    destinations:
      ...
      # defaults to .simple_e/warehouse.duckdb when this block is omitted.
      duckdb:
...
A [simpl.E](https://github.com/albinasplesnys/simple_e) extract-load project.

## Layout

- `simple_e_project.yml` - project config (committed).
...
- `.simple_e/` - disposable working dir (gitignored).

## Getting started

```bash
simple-e new connector my_source   # scaffold a source connector
simple-e validate                  # discovery-time validation
simple-e run -c my_source          # extract + load
```
"""
...
# {name} - a simpl.E SOURCE connector (docs/03).
name: {name}
kind: source
version: "0.1.0"
...
_DEST_REGISTER_YML = """\
# {name} - a simpl.E DESTINATION connector (docs/05).
name: {name}
kind: destination
version: "0.1.0"
...
commit_state -> close. See simple_e/destinations/duckdb/ for a complete,
...
def scaffold_project(directory: Path, *, force: bool = False) -> Path:
    """Write a new simpl.E project tree into ``directory`` — docs/06.

    Creates ``simple_e_project.yml``, ``profiles.yml``, empty ``connectors/``
    and ``destinations/`` folders, ``.gitignore`` and a short ``README.md``.
    Refuses to clobber an existing project (a ``simple_e_project.yml`` already
    ...
    name = directory.resolve().name or "simple_e_project"
