...
# engine (dtex/engine/config.py::build_config) constructs a single
...
the engine injects a :class:`~dtex.types.Cursor` (docs/03 §3.2). Both
...
from dtex.sources.postgres.client import (
    close_cursor_sql,
    connect,
    declare_cursor_sql,
    fetch_forward_sql,
...
def _extract_full_scan(
    *,
    ...
    """
    batch_size = int(config.batch_size)
    cursor_name = f"_dtex_pg_{uuid.uuid4().hex[:12]}"
