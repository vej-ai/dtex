...
(``import detx``, ``detx list``, the DuckDB destination, every test that does
...
# google-cloud-bigquery + google-cloud-storage are in detx's base dependencies
...
# with `pip install detx` (or `pip install --force-reinstall google-cloud-bigquery
# google-cloud-storage`).
_INSTALL_HINT = (
    "BigQuery destination requires google-cloud-bigquery + google-cloud-storage "
    "(both ship with `pip install detx`). If you see this, the SDK was removed "
    "from your environment — reinstall with `pip install detx`."
)
...
def _bigquery_module() -> Any:
    ...
    detx`` without ``[bigquery]`` installed does not crash. A missing SDK
...
def _service_account_credentials(path: str) -> Any:
    ...
    Credentials are *referenced* by path on disk — detx never logs the path's
...
class BigQueryClient:
    ...
    The two clients are built by :func:`build_client` from a :class:`~detx.types.Config`.
    """
...
class BQConn:
    ...
    * :attr:`runs_table_ready` — same flag for ``_detx_runs`` (docs/09 §4).
    """
...
# --------------------------------------------------------------------------
# Client construction — from a detx Config to a live BigQueryClient
# --------------------------------------------------------------------------
