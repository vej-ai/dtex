...
* :func:`scaffold_project` — a project tree with ``detx_project.yml``,
...
_PROJECT_YML = """\
# {name} - a detx project (docs/06 post-8.B).
#
...
A [detx](https://github.com/albinasplesnys/detx) extract-load project.

## Layout

- `detx_project.yml` - project config (committed).
...
- `.detx/` - disposable working dir (gitignored).

## Getting started

```bash
detx new source my_source        # scaffold a source connector
detx new config my_pipeline      # scaffold a configs/my_pipeline.yml
detx validate                    # discovery-time validation
detx run -p my_pipeline          # extract + load
```
"""
...
# {name} - a detx SOURCE connector (docs/03).
name: {name}
kind: source
version: "0.1.0"
...
_DEST_REGISTER_YML = """\
# {name} - a detx DESTINATION connector (docs/05).
name: {name}
kind: destination
version: "0.1.0"
...
commit_state -> close. See detx/destinations/duckdb/ for a complete,
...
_CONFIG_YML = """\
# {name} - a detx pipeline config (docs/12).
#
# One config = one source + one destination + one target + the params that
# customize both ends. Run with `detx run -p {name}`.
name: {name}
source: my_source            # rename me to a source under sources/
destination: duckdb
target: dev
...
def scaffold_project(directory: Path, *, force: bool = False) -> Path:
    """Write a new detx project tree into ``directory`` — docs/06 post-8.B.

    Creates ``detx_project.yml``, ``profiles.yml`` (destination-keyed),
    ...
    ``detx_project.yml`` already present) unless ``force`` is set. Returns the
    project root.
    """
    ...
    name = directory.resolve().name or "detx_project"
