...
config; the engine resolves it to a :class:`~detx.types.PipelineConfig` and
runs its lifecycle. This module owns:

* **Discovery** — :func:`discover_configs` walks each
  :class:`~detx.engine.config.ProjectConfig.config_paths` directory (default:
...
* **Lookup** — :func:`load_config` resolves one config by name through the
  same discovery, raising a clear :class:`~detx.engine.config.ConfigError` for
