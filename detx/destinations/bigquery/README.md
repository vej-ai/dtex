# BigQuery destination — detx v1

The production warehouse detx was always meant to land in. Tier A: BigQuery
hosts its own state and audit tables (`_detx_state`, `_detx_runs`) alongside
...
`google-cloud-storage`, and `pyarrow` ship in detx's base dependencies.
...
2. **Application Default Credentials (ADC)** — leave `credentials_path`
   empty (the default). detx then picks up `GOOGLE_APPLICATION_CREDENTIALS`,
...
detx never logs credentials. Service-account paths may appear in error
...
bigquery:
  ...
  targets:
    prod:
      ...
      staging_bucket: my-detx-staging   # REQUIRED (you create it; detx never does)
      location: US                     # default "US"
      staging_prefix: detx/staging      # default
...
(detx cleans up on success; on failure it leaves the Parquet for forensics).
...
BigQuery has no general `BEGIN` / `COMMIT` spanning multiple jobs. detx v1
...
`detx state reset -p <config>`; tomorrow: `detx state reset --recreate-table`,
...
- `_detx_state` — one row per `(connector, stream)` resume point. Eight
...
- `_detx_runs` — one row per run; `run_id` is the upsert key.
