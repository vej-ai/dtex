...
* **What does a SOURCE name resolve to?** :func:`resolve_source` turns a name
  ...
  ``sources/<name>/`` wins over the baked ``det/sources/<name>/``.
* **What does a DESTINATION name resolve to?** :func:`resolve_destination`
  ...
  ``destinations/<name>/`` then baked ``det/destinations/<name>/``.
...
def _baked_source_dir() -> Path:
    """Return the baked-sources root shipped inside the ``det`` package.

    docs/03 §5 / docs/06 post-8.B: baked *sources* live under ``det/sources/``
    (renamed from ``det/connectors/`` in stage 8.B). A custom source's
...
def _baked_destination_dir() -> Path:
    """Return the baked-destinations root shipped inside the ``det`` package.

    docs/03 §5 / docs/06: baked *destinations* live under ``det/destinations/``;
...
def find_source_folder(
    ...
    2. The baked source root (``det/sources/``).
...
def find_destination_folder(
    ...
    baked ``det/destinations/``. Project-local shadows baked (docs/03 §5).
    """
    searched: list[Path] = []
