...
det binds `streams[].name` in `register.yaml` to a `@stream(name=...)`
...
The YAML carries only what the det contract supports (name / table /
...
The det contract type `ParamSpec` (docs/03 §2.4) only allows scalar
