...
* end-to-end ``dtex.run`` into a tmp DuckDB
...
from dtex.sources.shiphero.client import ShipHeroClient, derive_auth_url
from dtex.sources.shiphero.pagination import (
    extract_records,
    paginate,
    walk_field_path,
)
from dtex.sources.shiphero.source import extract_stream
from dtex.sources.shiphero.windows import (
    compute_start,
    date_windows,
    to_utc_dt,
)
...
def test_extract_stream_first_run_starts_from_initial_value(
    ...
    cursor = Cursor(
        ...
        cursor_type=dtex.CursorType.TIMESTAMP,
...
# --------------------------------------------------------------------------
# End-to-end: dtex.run against a stubbed server, into a tmp DuckDB
# --------------------------------------------------------------------------


@pytest.fixture
def shiphero_project(tmp_path: Path) -> Path:
    """Build a throwaway dtex project that runs the shiphero baked source.

    Writes dtex_project.yml (post-8.B keys), a destination-keyed profiles.yml
...
def test_end_to_end_run_lands_shipments_into_duckdb(
    ...
    """``dtex.run`` discovers shiphero, runs all 3 streams, lands rows.
    ...
    try:
        ...
        # _dtex_synced_at populated.
        nulls = conn.execute(
            "SELECT count(*) FROM shipments WHERE _dtex_synced_at IS NULL"
        ).fetchone()
