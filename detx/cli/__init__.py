...
parses arguments, calls the engine's *public* surface — :func:`detx.run`,
the discovery functions in :mod:`detx.engine.discovery`, and the config loader
in :mod:`detx.engine.configs` — and formats the result. There is **no** run
...
longer runnable because it doesn't say where to write. ``detx list`` /
``detx validate`` cover sources, destinations, AND configs; ``detx init``
scaffolds the new layout (``sources/`` + ``destinations/`` + ``configs/``);
``detx new`` has three subcommands (``source``, ``destination``, ``config``);
``detx state`` is keyed by config (which resolves to a source for the actual
state lookup).

Command surface:

* ``detx run -p <config> [--target T] [--select S] [--full-refresh]
  [--param k=v] [--destination-param k=v]`` — extract + load one config.
* ``detx list [--kind {{source,destination,config}}]`` — list discoverable
  components.
* ``detx validate`` — discovery-time validation of every source, destination,
  and config.
* ``detx init [<dir>]`` — scaffold a new project tree.
* ``detx new {{source,destination,config}} <name>`` — scaffold one folder/file.
* ``detx state list -p <config>`` / ``detx state reset -p <config>
  [--stream S]`` — inspect / clear incremental state.
* ``detx --version`` — print the version.
"""
...
import detx
from detx.cli._discovery import (
    discover_all_configs,
    discover_all_destinations,
    discover_all_sources,
)
from detx.cli._format import (
    print_multi_run_summary,
    print_run_result,
    render_table,
)
from detx.cli._runs import get_run as runs_get
from detx.cli._runs import list_runs as runs_list
from detx.cli._runs import read_log_lines
from detx.cli._scaffold import (
    ScaffoldError,
    scaffold_config,
...
from detx.cli._secrets import check_project as _check_secrets_project
from detx.cli._state import StateError, list_state, reset_state
from detx.engine import ConfigError, DiscoveryError, EngineError
from detx.engine import config as cfg
from detx.engine import discovery as disc
from detx.types import RunStatus
...
@click.version_option(version=detx.__version__, prog_name="detx")
def cli() -> None:
    """detx — a simple, open-source extract-load tool.

    Move data from a source into a destination, and nothing more. Run
    ``detx <command> --help`` for command-specific options.
    """
    ...
    if tag is not None:
        ...
        if params:
            _fail(
                ...
                "apply to every matched config silently); use `detx run -p "
...
@click.option(
    ...
    "tags: drives `detx run --tag`; source/destination tags are catalog "
    "metadata only).",
)
@click.option(
    "--project-dir",
...
def list_components(
    ...
    ``detx run --tag`` selects on). A section with no matches still shows
...
@click.option(
    "--force", is_flag=True, help="Overwrite an existing detx_project.yml."
)
def init(directory: Path, force: bool) -> None:
    """Scaffold a new detx project in DIRECTORY (default: current directory).

    Writes ``detx_project.yml``, ``profiles.yml`` (destination-keyed),
    ...
    try:
        ...
        _fail(str(exc), code=2)
        return  # unreachable.
    click.echo(click.style(f"created detx project at {root}", fg="green"))
    click.echo("next:")
    click.echo("  detx new source my_source")
    click.echo("  detx new config my_pipeline")
    click.echo("  detx validate")
    click.echo("  detx run -p my_pipeline")
...
def new_source(name: str, project_dir: Path | None) -> None:
    ...
    click.echo("  detx new config <name>   # bind this source to a destination")
    click.echo("  detx validate")
...
@cli.group()
def state() -> None:
    """Inspect and reset incremental state (the destination's _detx_state)."""
...
def state_list(
    ...
    """Show the ``_detx_state`` rows for one config's source.
...
def state_reset(
    ...
    Deletes the rows in ``_detx_state`` for the config's source (or one
...
@cli.group()
def runs() -> None:
    """Inspect run history (the destination's _detx_runs + per-run JSONL log)."""
...
def runs_list_cmd(
    ...
    """List recent runs from the destination's ``_detx_runs`` table.
...
def runs_show_cmd(
    ...
    Looks up ``<run_id>`` in ``_detx_runs`` (the queryable summary) and
    reads ``.detx/logs/<run_id>/run.jsonl`` (the narrative). Accepts the
    ...
    if record is not None:
        ...
        click.echo(click.style(f"no _detx_runs row for {run_id!r}", fg="yellow"))
...
def main() -> None:
    """Entry point for the ``detx`` console script (pyproject scripts).
