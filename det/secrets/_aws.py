...
* ``<secret-id>`` — the secret name (or full ARN). Opaque to det.
...
— no det-side credential argument is passed.
...
2. Grant the IAM principal det runs as the
...
# path. Surfacing a base64-decoded binary as a det secret value would
# also conflict with downstream consumers that expect ``str``. A binary
# secret raises :class:`~det.secrets.SecretResolutionError` with a clear
...
# exponential backoff; layering det-side retries would double the
...
def _lazy_import_boto3() -> Any:
    ...
    imported at ``det`` package import time, so a base install (or one
...
class AwsSecretsManagerResolver:
    ...
    * Malformed path (no slash, empty region, invalid region characters,
      empty secret-id) → :class:`~det.secrets.SecretResolutionError`
