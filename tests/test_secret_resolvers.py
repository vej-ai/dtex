"""Tests for the ``det.secrets`` package — stage 9a.
...
import det
from det.engine.logger import Redactor, build_logger
...
# ---------------------------------------------------------------------------
# Project-local det_plugins.py
# ---------------------------------------------------------------------------


def test_project_plugins_file_runs_registrations(tmp_path: Path) -> None:
    """A ``det_plugins.py`` next to ``det_project.yml`` is imported, and its
    ``register_secret_resolver`` calls take effect."""
    (tmp_path / "det_project.yml").write_text("name: t\n")
    (tmp_path / "det_plugins.py").write_text(
        "from typing import ClassVar\n"
        "import det\n"
        "\n"
        "class P:\n"
...
def test_project_plugins_idempotent(tmp_path: Path) -> None:
    ...
    (tmp_path / "det_plugins.py").write_text(
        ...
        "        return 'v'\n"
        "\n"
        "det.register_secret_resolver('p1', P)\n"
    )
    load_project_plugins(tmp_path)
...
def test_project_plugins_missing_is_noop(tmp_path: Path) -> None:
    """A project without ``det_plugins.py`` is fine — no error."""
    (tmp_path / "det_project.yml").write_text("name: t\n")
    load_project_plugins(tmp_path)  # No raise.


def test_project_plugins_import_error_wrapped(tmp_path: Path) -> None:
    """A ``det_plugins.py`` that raises at import surfaces as
    :class:`SecretResolutionError` with the underlying exception chained."""
    (tmp_path / "det_project.yml").write_text("name: t\n")
    (tmp_path / "det_plugins.py").write_text("raise RuntimeError('boom')\n")
    with pytest.raises(SecretResolutionError) as exc_info:
        load_project_plugins(tmp_path)
    assert "det_plugins.py" in str(exc_info.value)
    assert isinstance(exc_info.value.__cause__, RuntimeError)
...
def test_public_api_exports() -> None:
    ...
    assert det.SecretResolver is SecretResolver
    assert det.SecretResolutionError is SecretResolutionError
    assert det.register_secret_resolver is register_secret_resolver
...
def test_secret_ref_accepts_secret_url() -> None:
    from det.types import SecretRef
...
def test_resolve_secret_ref_dispatches_secret_url(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from det.engine.config import Profiles, resolve_secret_ref
    from det.types import SecretRef
...
def test_resolve_secret_ref_secret_url_failure_wrapped() -> None:
    ...
    from det.engine.config import ConfigError, Profiles, resolve_secret_ref
    from det.types import SecretRef
...
def teardown_module(module: object) -> None:
    ...
    for name in list(logging.Logger.manager.loggerDict):
        if name.startswith("det.run.test-"):
