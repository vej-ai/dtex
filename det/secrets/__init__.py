...
(:func:`resolve_secret_url`), the project-local ``det_plugins.py``
...
package adds itself with a tiny ``[project.entry-points."det.secret_resolvers"]``
block — no engine change.

# NOTE: the existing ``${env.X}`` / ``${profile.X.Y}`` resolution lives in
# :mod:`det.engine.config` and stays unchanged. Those are string-interpolation
...
Public API (re-exported from :mod:`det`):
...
* :func:`resolve_secret_url` — the parser + dispatch used by
  :mod:`det.engine.config`.
* :func:`is_secret_url` — the dispatch predicate.
* :func:`load_project_plugins` — the project-local ``det_plugins.py``
  importer (called once per project from :mod:`det.engine.discovery`).
...
from det.secrets.resolvers import (
