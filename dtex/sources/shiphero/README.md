...
A dtex source connector that extracts ShipHero fulfillment data via the
...
- `batch_size` (default 200) — records per dtex batch (the destination
...
```sh
SHIPHERO_REFRESH_TOKEN=... dtex run shiphero --target dev \
...
- `get_checkpoint` / `save_checkpoint` → the engine's `Cursor` + `_dtex_state`.
