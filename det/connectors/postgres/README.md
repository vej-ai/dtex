...
A det source connector that extracts rows from a Postgres database. Each
...
`det/connectors/postgres/`. A user project can shadow it by dropping a
...
`schema:`) maps to one of det's `FieldType` members (docs/03 §2.2.1):
...
`application_name` (default `det`), `connect_timeout_seconds` (default
