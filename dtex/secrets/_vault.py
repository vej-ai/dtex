...
* ``<mount-path>/<kv-path>`` — the full Vault read path, opaque to dtex.
...
* ``<field>`` — REQUIRED. Vault's KV engines return a JSON object;
  ...
  raises a clear :class:`~dtex.secrets.SecretResolutionError`.

Auto-registered via ``[project.entry-points."dtex.secret_resolvers"]``::

    pip install 'dtex[vault]'

The resolver is built on first use by stage 9a's lazy instantiation
(:func:`dtex.secrets.resolvers._get_resolver`); the :class:`hvac.Client`
...
2. Grant the token dtex runs as a policy with
...
4. Export ``VAULT_ADDR`` (the Vault URL) and ``VAULT_TOKEN`` (the auth
   token) in the environment dtex runs in. These are the same env vars
...
def _lazy_import_hvac() -> Any:
    ...
    imported at ``dtex`` package import time, so a base install keeps
...
class VaultResolver:
    ...
    * Effective lifetime: one ``hvac.Client`` per ``dtex`` process,
      however many ``secret://vault/...`` references the run consumes.

    Errors:

    * Missing ``#field`` → :class:`~dtex.secrets.SecretResolutionError`
      with the contract message — fired BEFORE SDK import.
    * Empty path → :class:`~dtex.secrets.SecretResolutionError` — fired
      BEFORE SDK import.
