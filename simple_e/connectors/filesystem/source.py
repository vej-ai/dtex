...
(:mod:`.readers`), and yields :class:`~simple_e.Batch` of records — with the
synthetic ``_simple_e_file_cursor`` field attached to every record.
...
extra field (``_simple_e_file_cursor``) on every record the source yields:

* the engine sees a normal cursor field — every existing mechanism
  (``Cursor.observe``, ``_simple_e_state``, ``cursor.start_value`` filtering
...
# The synthetic record field (``_simple_e_file_cursor``) is always the
...
# ``_simple_e_state.cursor_value``) is the ``cursor_observe`` value: a
...
# (see simple_e/connectors/shiphero/source.py lines 154-165). This bug
...
from simple_e.connectors.filesystem.backends import FileRef, pick_backend
from simple_e.connectors.filesystem.readers import infer_format, reader_for_format
...
FILE_CURSOR_FIELD = "_simple_e_file_cursor"
...
def files(config: Config, cursor: Cursor) -> Iterator[Batch]:
    ...
    Every yielded record carries ``_simple_e_file_cursor`` — the file's
    ...
    ``_simple_e_state``, so the next run resumes past every loaded file.
    """
    ...
    # the value comes back from `_simple_e_state.cursor_value` (a JSON
