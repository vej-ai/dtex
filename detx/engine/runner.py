...
RUN STREAMS → RUN RECORD — and returns a :class:`~detx.types.RunResult`.
...
from detx.engine.logger import Redactor, RunLog, build_logger
from detx.registry import compute_injection
...
def _format_traceback(exc: BaseException) -> str:
    ...
    The full traceback lands in ``.detx/logs/<run_id>/run.jsonl`` (the
    forensics surface — docs/09 §3.2); deliberately NOT in the ``_detx_runs``
...
def run_tag(
    ...
    * **Order**: alphabetical by config name. Predictable, stable across
      ...
      :func:`detx.cli._discovery.discover_all_configs` which already returns
      sorted output, so the order matches ``detx list --kind config``. The
    ...
    * **Parallelism (stage 8e)**: ``threads`` is the project-wide
    ...
    # overrides should call ``detx run -p <config> --param k=v`` per
    ...
    # The verification step ``detx run --tag test
    ...
    # here rather than reaching into ``detx.cli._discovery.discover_all_configs``
...
# ``detx run --tag`` invocation is one process from the user's shell, so
# the global is safe.
#
...
def run(
    ...
    *,
    ...
    as :func:`detx.run`). It executes one synchronous pass:

    1. **DISCOVER** — find the project root (``project_dir`` or walk up for
       ``detx_project.yml``); load ``configs/`` and look up ``config``; resolve
    ...
    * ``select`` — when non-empty, *replaces* the config's ``select:`` (the CLI
    ...
    try:
        # -- Stage 1: DISCOVER ----------------------------------------------
        project_root = disc.find_project_root(project_dir)
        # Project-local plugin file: ``detx_plugins.py`` next to
        # ``detx_project.yml`` (stage 9a — docs/08 §3). The file (if present)
        # calls ``detx.register_secret_resolver(...)`` to register custom
        ...
        # a FAILED :class:`RunResult`. See :func:`detx.secrets.load_project_plugins`.
        load_project_plugins(project_root)
        project = cfg.ProjectConfig.load(project_root)
        profiles = cfg.Profiles.load(project_root)
        ...
        # ``.detx/logs/`` is per-project; the dir is created lazily.
        run_log = RunLog(run_id, project_root / ".detx" / "logs", redactor)
        ...
        if source.manifest.destination is not None:
            logging.getLogger("detx.engine").warning(
