...
1. Declare it under ``streams:`` in ``register.yaml`` (name / table /
   ...
   only what the det contract supports.
2. Write a ``@stream(name="X")`` function here that calls
   ...
   live in ``register.yaml`` because :class:`~det.types.ParamSpec` is
...
# contract type :class:`~det.types.ParamSpec` only accepts scalar values,
...
def extract_stream(
    *,
    ...
    4. For incremental streams, sends the cursor's
       :meth:`~det.types.Cursor.start_value` as
