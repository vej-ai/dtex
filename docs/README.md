# simpl.E — Design Handbook

**simpl.E** (short for "simple extract") is an open-source, self-hosted Python **extract-load (EL)** tool. The pitch in one line: *dlthub meets dbt* — and explicitly **not Airbyte** (no UI blackbox).
...
|---|---|---|
| 00 | [Vision & Naming](./00-vision-and-naming.md) | Why simpl.E exists, the "dbt for EL" thesis, the simplicity mandate, and the naming. |
| 01 | [Landscape & Comparison](./01-landscape-and-comparison.md) | Where simpl.E sits vs. Airbyte, Fivetran, dlt, Meltano, and custom scripts. |
...
| 05 | [Destinations and State](./05-destinations-and-state.md) | The destination interface, the pre-baked catalog, schema handling, write dispositions, and the `_simple_e_state` design with the capability-tier model. |
| 06 | [Project Anatomy](./06-project-anatomy.md) | `simple_e_project.yml`, the `connectors/` and `destinations/` directories, `.simple_e/`, tags, targets. |
| 07 | [CLI and Library API](./07-cli-and-library-api.md) | The full `simple_e` command surface, exit codes, the importable Python library, orchestrator use, config precedence. |
