...
of the detx connector contract. It contains *only* type definitions and their small intrinsic
...
class _StrEnum(StrEnum):
    ...
    def parse(cls, value: Any) -> Any:
        ...
        which is the message ``detx validate`` surfaces to the author.
        """
...
class Capability(Enum):
    ...
    # ``RUN_RECORDS`` (docs/09 §4) to gate the ``_detx_runs`` audit table
    ...
    STATE = "state"
    """Destination can host the ``_detx_state`` table itself (Tier A)."""
    MERGE = "merge"
    ...
    """Destination can host the ``_detx_runs`` audit table itself (docs/09 §4).
...
class CursorType(_StrEnum):
    ...
    examples and the ``_detx_state.cursor_type`` column.
    """
...
def _parse_partition_by(value: Any) -> str | PartitionConfig | None:
    ...
    * a string — the short form (column name). Stored verbatim; the engine
      ...
      cursor type in hand (see the resolver's NOTE in :mod:`detx.engine.runner`).
...
class Schema:
    ...
    SYNCED_AT_COLUMN: ClassVar[str] = "_detx_synced_at"
    ...
    def synced_at_field() -> Field:
        """The engine-appended ``_detx_synced_at`` column — docs/03 §2.2.1.
    ...
    def with_synced_at(self) -> Schema:
        """Return a new :class:`Schema` with ``_detx_synced_at`` appended.
...
class SecretRef:
    ...
    # ships as a plugin without an engine change. See :mod:`detx.secrets`.
    ...
    def is_valid_ref(cls, ref: str) -> bool:
        ...
        * ``secret://<scheme>/<path>[#<field>]`` — pluggable secret manager
          ...
          :func:`detx.secrets.resolve_secret_url` — this surface check only
...
class StreamDef:
    ...
    * ``None`` (no declaration — the engine may compute a cursor-based
    ...
    # NOTE in :mod:`detx.engine.runner`)."""
...
class State:
    ...
    ``state_blob`` JSON column of ``_detx_state`` after batches durably
...
class StateBackend(Protocol):
    ...
    Object-storage destinations cannot host the ``_detx_state`` table, so
...
@dataclass
class StateRecord:
    """One ``_detx_state`` row — the per-stream resume point.
    ...
    # excluded — it is recoverable by joining ``_detx_runs`` on
    ...
    def to_row(self) -> dict[str, Any]:
        """Serialize to a plain dict matching the ``_detx_state`` columns.
    ...
    def from_row(cls, row: Mapping[str, Any]) -> StateRecord:
        """Deserialize a ``_detx_state`` row read back from a destination.
...
class RunConfig:
    ...
    # ``_detx_state.connector`` column is keyed by source so a re-run of a
...
class RunResult:
    ...
    run record (``_detx_runs`` row / ``.detx/runs/<run_id>.json``).
...
@dataclass(frozen=True)
class RunRecord:
    """One row in ``_detx_runs`` — the queryable audit record of one run.
    ...
    *receipt* — the destination-persisted summary one row of ``_detx_runs``
    ...
    # bloat every ``_detx_runs`` row in a way that confounds SQL drill-down.
...
class PipelineConfig:
    ...
    * ``params`` — source param overrides (a higher precedence layer than
      ``register.yaml`` defaults and ``detx_project.yml`` ``vars``; lower
    ...
    * ``tags`` — a bare list of strings used by ``detx run --tag <tag>`` to
      select every config that carries the tag (and by ``detx list --tag``
      ...
      ``detx list``, never by ``detx run --tag``. Clean separation: source/
    ...
    schedule: str | None = None
    tags: tuple[str, ...] = ()
    """Free-form labels used by ``detx run --tag <tag>`` to select every config
    that carries the tag (and by ``detx list --tag`` for catalog filtering).
    ...
    that ``detx run --tag`` deliberately ignores)."""
