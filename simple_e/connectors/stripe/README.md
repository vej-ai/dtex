...
land as `JSON` columns — the simpl.E convention for nested data
(docs/04 §"Nested data").
...
of arrival order. Per simpl.E's commit model the cursor is committed only
...
`default_destination`. Running it is the standard simpl.E one-liner:
...
```sh
simple-e run stripe --target prod --select charges
```
