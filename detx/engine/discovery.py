...
* **Where is the project?** :func:`find_project_root` walks up from a starting
  directory for ``detx_project.yml`` (docs/02, docs/06).
* **What does a SOURCE name resolve to?** :func:`resolve_source` turns a name
  ...
  :class:`~detx.registry.ConnectorRegistry` the connector's decorators
  populated. Resolution order is fixed (docs/03 §5): project-local
  ``sources/<name>/`` wins over the baked ``detx/sources/<name>/``.
* **What does a DESTINATION name resolve to?** :func:`resolve_destination`
  ...
  ``destinations/<name>/`` then baked ``detx/destinations/<name>/``.
...
:func:`~detx.registry.registration_scope`, exec every ``.py`` file under a
...
class DiscoveryError(Exception):
    ...
    runner converts it into a ``FAILED`` :class:`~detx.types.RunResult`
...
class LoadedConnector:
    ...
    ``register.yaml`` (:class:`~detx.types.ConnectorManifest`) and the
    :class:`~detx.registry.ConnectorRegistry` the connector's decorators
...
def find_project_root(start: Path | str | None = None) -> Path:
    """Walk up from ``start`` to the directory holding ``detx_project.yml``.

    docs/02: "find the project root (walk up for ``detx_project.yml``)".
    ...
    raise DiscoveryError(
        ...
        f"a detx project must contain a {PROJECT_FILE} (docs/06)"
    )
...
def _baked_source_dir() -> Path:
    """Return the baked-sources root shipped inside the ``detx`` package.

    docs/03 §5 / docs/06 post-8.B: baked *sources* live under ``detx/sources/``
    (renamed from ``detx/connectors/`` in stage 8.B). A custom source's
...
def _baked_destination_dir() -> Path:
    """Return the baked-destinations root shipped inside the ``detx`` package.

    docs/03 §5 / docs/06: baked *destinations* live under ``detx/destinations/``;
...
def find_source_folder(
    ...
    2. The baked source root (``detx/sources/``).
...
def find_destination_folder(
    ...
    baked ``detx/destinations/``. Project-local shadows baked (docs/03 §5).
    """
    searched: list[Path] = []
...
def _import_module_from_path(path: Path) -> None:
    ...
    """
    unique_name = f"_detx_connector_{path.stem}_{uuid.uuid4().hex}"
...
def _load_connector_folder(folder: Path) -> LoadedConnector:
    ...
    :func:`~detx.registry.registration_scope`, so every ``@stream`` /
